...
캐싱은 자주 접근하는 데이터를 빠른 저장소에 임시로 보관해 조회 비용을 줄이는 방법이다. 다만 원본과 캐시를 따로 관리하는 만큼, 언제 데이터를 저장하고 갱신할지에 따라 성능과 데이터 일관성이 달라진다.
...
Cache-Aside는 애플리케이션이 캐시와 데이터베이스에 직접 접근하는 패턴으로, 널리 사용되는 방식이다. 캐시 조회부터 원본 데이터 로드, 캐시 갱신까지 애플리케이션이 관리한다.

### 동작 방식

읽을 때는 먼저 캐시를 조회한다. 데이터가 있으면 그대로 반환하고(Cache Hit), 없으면 DB에서 조회한 결과를 캐시에 저장한다(Cache Miss).

쓸 때는 데이터베이스를 먼저 갱신한다. 이후 관련 캐시를 삭제하거나 새 값으로 바꿔, 다음 조회에서 이전 값이 계속 사용되지 않도록 한다.
...
- 장점: 구현이 간단하고 유연함:
  - 단점: Cache Miss 시 지연 발생
- 장점: 필요한 데이터만 캐싱 (메모리 효율적):
  - 단점: 캐시와 DB 간 데이터 불일치 가능
- 장점: 캐시 장애 시에도 서비스 가능:
  - 단점: 애플리케이션에서 캐시 로직 관리 필요
- 장점: 읽기 부하가 높은 시스템에 적합:
  - 단점: Cold Start 문제 (초기 캐시 비어있음)
...
Cache-Aside에서 애플리케이션이 맡았던 데이터 로드 책임을 캐시 계층으로 옮긴 방식이 Read-Through다. 애플리케이션은 캐시에 데이터를 요청하고, 캐시 계층이 필요할 때 DB에서 데이터를 가져와 저장한다.

### 동작 방식

- 애플리케이션이 캐시에 데이터 요청
- 캐시에 데이터가 있으면 반환
- 캐시에 없으면 캐시가 DB에서 직접 로드하여 저장 후 반환
...
- 특성: 캐시 접근 주체:
  - Cache-Aside: 애플리케이션
  - Read-Through: 캐시 라이브러리/프레임워크
- 특성: 코드 복잡도:
  - Cache-Aside: 높음 (직접 구현)
  - Read-Through: 낮음 (추상화됨)
- 특성: 유연성:
  - Cache-Aside: 높음
  - Read-Through: 상대적으로 낮음
- 특성: 캐시 로직 위치:
  - Cache-Aside: 비즈니스 로직에 섞임
  - Read-Through: 인프라 레이어에 분리

## Write-Through

### 개념

Write-Through는 쓰기 요청을 캐시와 데이터베이스 양쪽에 동기적으로 반영한다. 두 곳의 저장이 모두 성공해야 완료 응답을 보내므로, 쓰기 경로에 DB 저장 시간이 포함된다.

### 동작 방식

- 애플리케이션이 캐시에 쓰기 요청
- 캐시가 데이터를 저장하고 동시에 DB에도 저장
- 둘 다 성공하면 완료 응답
...
- 장점: 캐시와 DB 간 데이터 일관성 보장:
  - 단점: 쓰기 지연 시간 증가 (두 곳에 저장)
- 장점: 읽기 시 항상 최신 데이터:
  - 단점: 쓰기 처리량 제한
- 장점: 데이터 손실 위험 낮음:
  - 단점: 캐시 장애 시 쓰기도 실패 가능
...
Write-Behind는 캐시에 데이터를 저장하면 먼저 응답하고, DB에는 나중에 비동기로 반영한다. DB 저장을 기다리지 않아 쓰기 응답이 빠르지만, 반영 전에 캐시에 장애가 나면 데이터를 잃을 수 있다.
...
- 캐시 무효화는 "컴퓨터 과학에서 가장 어려운 문제 중 하나"로 알려져 있음.
...
TTL 만료를 기다리기 어려운 데이터는 변경 이벤트에 맞춰 캐시를 무효화할 수 있다. 아래 예시는 상품 가격이 바뀌면 상품 상세와 관련 목록의 캐시 키를 이벤트로 발행한다.
...
관련 키를 일일이 삭제하는 대신, 캐시 키에 포함된 버전을 바꾸는 방법도 있다. 아래 예시에서는 엔티티의 버전 번호를 올려 이후 조회가 새로운 키를 사용하도록 한다.
...
- 전략: TTL:
  - 장점: 구현 간단, 자동 정리
  - 단점: 데이터 불일치 가능
  - 적합한 사용 사례: 정적 콘텐츠, 설정 데이터
- 전략: 이벤트 기반:
  - 장점: 즉시 무효화, 정확함
  - 단점: 구현 복잡, 인프라 필요
  - 적합한 사용 사례: 실시간성 중요한 데이터
- 전략: 버전 기반:
  - 장점: 일괄 무효화 간단
  - 단점: 버전 관리 필요
  - 적합한 사용 사례: 자주 변경되는 엔티티
- 전략: 태그 기반:
  - 장점: 유연한 그룹 무효화
  - 단점: 메모리 오버헤드
  - 적합한 사용 사례: 연관된 데이터 그룹
