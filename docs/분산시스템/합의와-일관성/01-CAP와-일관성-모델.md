...
노드 사이의 통신이 끊겼을 때도 요청에 응답할 것인지, 다른 노드와 값을 맞출 때까지 기다릴 것인지 결정해야 한다. CAP 정리는 이때 일관성과 가용성을 동시에 보장할 수 없는 이유를 설명한다. PACELC는 분할이 없는 상황에서도 지연 시간과 일관성을 함께 고려하게 한다.

- "분산 시스템에서 완벽함은 없음. 트레이드오프를 이해하고 비즈니스 요구사항에 맞는 선택을 해야 함." - Release It!

## CAP 정리 상세

### CAP 정리란?

CAP 정리는 2000년 Eric Brewer가 제안하고 2002년 Gilbert와 Lynch가 증명했다. 비동기 분산 시스템에서 네트워크 분할을 허용하면 선형화 가능한 일관성과 모든 비장애 노드 요청에 대한 응답을 동시에 보장할 수 없다는 내용이다. 이 제약은 분할이 발생한 상황에 대한 것이며, 분할이 없을 때도 항상 둘 중 하나를 포기해야 한다는 뜻은 아니다. [Gilbert와 Lynch의 증명](https://www.cs.princeton.edu/courses/archive/spr22/cos418/papers/cap.pdf)
...
- 의미: 분산 시스템에서는 필수적으로 보장해야 함
- 현실: 네트워크 장애는 언제든 발생할 수 있음

Node 1, 2, 3이 통신하던 중 Node 3이 나머지와 연결되지 않는 상황을 생각해 보자. Partition A에는 Node 1과 2가, Partition B에는 Node 3이 남는다. 이때 일관성을 우선하는 CP 시스템은 일부 요청을 거부하고, 가용성을 우선하는 AP 시스템은 값이 달라질 수 있어도 응답을 계속한다.
...
CAP이 네트워크 분할 상황을 다룬다면, Daniel Abadi가 2010년에 제안한 PACELC는 분할이 없을 때의 선택까지 설명한다. 분할(Partition)이 발생하면 가용성(Availability)과 일관성(Consistency) 사이에서 선택하고, 그렇지 않을 때(Else)는 지연 시간(Latency)과 일관성(Consistency) 사이에서 절충한다.
...
최신 값을 즉시 읽는 보장과 별개로, 서로 의존하는 작업의 순서를 지켜야 하는 경우도 있다. 아래 예제에서는 게시물을 읽은 뒤 작성한 댓글에 게시물의 버전을 의존성으로 전달한다. 다른 노드에 댓글이 먼저 도착해도 게시물의 도착을 기다리는 방식이다.
