...
병목(Bottleneck)은 시스템 전체의 성능을 제한하는 구간이다. Michael Nygard의 "Release It!"에서 강조하듯이 한 구간의 병목을 해결하면 다른 구간이 새 병목으로 드러날 수 있다. 따라서 요청 경로에서 시간이 쓰이는 곳을 확인하고, 개선 후에도 같은 경로를 다시 살펴봐야 한다.

- "시스템은 가장 느린 구성 요소만큼만 빠름. 병목을 해결하면 다음 병목이 드러난다." - Release It!

애플리케이션 서버에서는 CPU와 메모리, 데이터베이스에서는 쿼리와 락 대기가 병목이 될 수 있다. 외부 API를 호출한다면 네트워크 I/O를, 캐시를 사용한다면 메모리 사용도 함께 확인한다.
...
쿼리 실행이 빨라도 커넥션을 얻기 위해 오래 기다리면 응답은 늦어진다. 아래 HikariCP 예제는 풀 크기와 타임아웃을 설정하고, 사용 중인 커넥션 수와 커넥션을 기다리는 스레드 수를 함께 기록한다.
...
반복 조회가 병목이라면 데이터를 다시 계산하거나 DB에서 읽는 횟수를 줄일 수 있다. 정적 리소스와 API 응답은 CDN이나 Edge Cache에, 세션과 인증 토큰은 Redis나 Memcached에, 자주 조회하는 설정은 Caffeine이나 Guava 같은 로컬 캐시에 두는 구성을 생각할 수 있다.
...
조회 결과를 캐시에 저장했다면 원본이 바뀔 때 어떤 캐시를 지울지도 정해야 한다. 아래 예제는 개별 키, 키 패턴, 태그로 삭제 대상을 선택하고, 여러 인스턴스의 로컬 캐시는 Redis Pub/Sub으로 무효화 요청을 전달한다.
...
응답을 만드는 데 여러 조회가 필요하다면 각 조회의 의존 관계를 먼저 확인한다. 아래 코드는 주문을 조회한 뒤 결제, 배송, 상품 정보를 각각 가져와 `thenCombine`으로 결과를 모은다.
...
같은 형태의 작업을 대량으로 실행할 때는 건별 호출 비용도 누적된다. 다음 예제는 JDBC의 배치 삽입, JPA의 주기적인 `flush`와 `clear`, 여러 주문의 상태를 바꾸는 Bulk Update를 비교한다.
