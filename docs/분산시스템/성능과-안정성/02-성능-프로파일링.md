...
성능을 개선하려면 먼저 어느 작업에 시간이 쓰이는지 확인해야 한다. 성능 프로파일링은 이 병목을 찾는 과정이다. Michael Nygard의 "Release It!"에서 강조하듯이 운영 환경의 데이터와 트래픽 패턴에서만 드러나는 문제가 있으므로, 실제 요청을 관측할 수 있어야 한다.

- "프로덕션에서 발생하는 성능 문제는 테스트 환경에서 재현하기 어렵다. 그러므로 운영 환경의 관측 가능성(Observability)이 중요함." - Release It!

## APM 도구

### APM(Application Performance Monitoring)이란?

APM은 애플리케이션의 성능과 가용성을 실시간으로 관측하고 분석하는 도구다. 메트릭(Metrics)으로 변화가 생긴 시점을 찾고, 트레이스(Traces)로 느려진 호출 구간을 좁힌 뒤 로그(Logs)에서 세부 상황을 확인할 수 있다. 이 데이터를 연결해 대시보드와 알림에 활용한다.
...
APM으로 느린 구간을 찾았다면 프로파일러로 그 안의 메서드 호출을 살펴본다. 아래 예제는 JFR, async-profiler, VisualVM을 사용하는 방법과 반복적인 문자열 연결 코드를 비교한다.
...
실행 시간뿐 아니라 객체가 얼마나 생성되고 유지되는지도 성능에 영향을 준다. 힙 덤프로 살아 있는 객체를 살펴보고, JFR의 할당 이벤트로 객체가 만들어지는 위치를 확인한다. 아래에는 컬렉션, 리스너, 커넥션이 해제되지 않는 예제도 함께 정리했다.
...
개선할 구간을 정할 때는 그 구간이 전체 실행 시간에서 차지하는 비중을 함께 봐야 한다. Amdahl의 법칙은 병렬화 가능한 비율을 `P`, 그 부분의 속도 향상 배수를 `S`라고 할 때 전체 속도 향상을 다음과 같이 나타낸다.

`Speedup = 1 / ((1 - P) + P/S)`

전체의 90%를 10배 빠르게 처리해도 `1 / (0.1 + 0.09) = 5.26배`까지만 빨라진다. 남은 10%의 직렬 구간이 전체 성능을 제한하기 때문이다.
...
넓고 평평한 영역(Plateau)은 시간이 많이 쓰이는 구간을 찾는 출발점이 된다. 반면 높고 좁은 영역(Tower)은 호출 스택이 깊다는 뜻이므로, 재귀 호출이나 프레임워크 내부 호출인지 살펴본다.

넓은 프레임이 JDBC에 몰려 있으면 쿼리를, JSON 처리에 몰려 있으면 직렬화를 확인한다. GC나 Lock 프레임이 넓다면 각각 메모리 사용과 동시성 문제를 살펴볼 수 있다. 수정한 뒤에는 같은 조건에서 수집한 Flame Graph를 비교해 해당 구간이 실제로 줄었는지 확인한다.
