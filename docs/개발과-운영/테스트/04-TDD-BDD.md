...
TDD(Test-Driven Development)는 테스트를 먼저 작성하고 그 테스트를 통과하는 코드를 구현하는 개발 방식이다. Kent Beck이 1990년대 후반 Extreme Programming의 일부로 개발했으며, 테스트와 구현, 리팩토링을 짧게 반복한다.

### 전통적 개발 vs TDD

구현 뒤에 테스트를 작성하는 흐름은 요구사항 분석, 설계, 구현, 테스트, 버그 수정 순서로 진행한다. TDD에서는 요구사항을 테스트로 먼저 표현하고, 구현과 리팩토링을 거친 뒤 다음 동작으로 이 과정을 반복한다.

### TDD의 핵심 원칙

- Kent Beck의 "Test-Driven Development by Example"에서 정의한 두 가지 규칙:

- 새로운 코드는 실패하는 자동화된 테스트가 있을 때만 작성함
- 중복을 제거함
...
- TDD를 통해 달성하고자 하는 상태:
- 기존에 동작하던 모든 것이 여전히 동작함
- 새로운 동작이 예상대로 작동함
- 시스템이 다음 변경을 받아들일 준비가 되어 있음
- 프로그래머와 동료들이 위 사항들에 확신을 가짐

## Red-Green-Refactor 사이클

### 개념

이 반복은 Red, Green, Refactor의 세 단계로 나뉜다. 실패하는 테스트로 필요한 동작을 드러내고, 최소 구현으로 테스트를 통과시킨 뒤, 동작을 유지하면서 코드를 정리한다.
...
구현에 들어가기 전에 테스트가 실제로 실패하는지 확인한다. 그래야 테스트가 실행되고 있으며, 아직 없는 동작을 제대로 검증하는지 알 수 있다.
...
BDD(Behavior-Driven Development)는 Dan North가 2006년에 제안한 개발 방법론으로, TDD를 비즈니스 관점으로 확장한다. 테스트를 설명할 때 함수나 메서드 자체보다 사용자가 기대하는 행동(Behavior)에 초점을 맞춘다.

### TDD vs BDD

예를 들어 "이 함수가 올바른 값을 반환하는가?", "이 메서드가 예외를 던지는가?"라는 기술적 질문을 "사용자가 로그인하면 대시보드를 볼 수 있음", "VIP 고객이 주문하면 10% 할인을 받음" 같은 행동으로 표현한다.

### Given-When-Then 패턴

행동을 구체적인 시나리오로 쓰려면 전제와 실행, 결과를 연결해야 한다. Given은 초기 상태와 전제 조건, When은 사용자의 행동, Then은 그 행동으로 기대하는 결과를 나타낸다.
