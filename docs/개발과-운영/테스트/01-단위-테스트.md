...
테스트 피라미드는 Mike Cohn이 "Succeeding with Agile"에서 소개한 개념으로, 자동화 테스트의 계층 구조를 나타낸다. 개별 함수나 메서드를 빠르고 낮은 비용으로 검증하는 단위 테스트를 바닥에 두고, 컴포넌트 간 상호작용을 확인하는 통합 테스트를 그 위에 둔다. 전체 시스템을 확인하는 E2E 테스트는 느리고 비용이 높아 가장 위에 배치한다.

아래에서는 단위 테스트 70%, 통합 테스트 20%, E2E 테스트 10%의 권장 비율로 이 구성을 비교한다.
...
반대로 E2E 테스트가 많고 단위 테스트가 적으면 피라미드를 뒤집은 아이스크림 콘 형태가 된다. 느리고 불안정한 테스트의 비중이 커져 실행 시간이 늘고, 실패했을 때 버그 위치를 파악하기도 어렵다. 그만큼 유지보수 비용도 증가한다.

## 테스트 더블: Mock, Stub, Spy, Fake

단위 테스트에서 외부 의존성을 그대로 사용하면 실행 속도와 결과가 그 의존성의 상태에 영향을 받는다. 테스트 더블은 이때 실제 객체를 대신하는 객체를 통칭하며, Gerard Meszaros가 "xUnit Test Patterns"에서 정리한 용어다. 대체 객체가 무엇을 반환하고 무엇을 검증하는지에 따라 Stub, Mock, Spy, Fake, Dummy로 구분한다.

### Stub (스텁)

- 정의: 미리 정해진 응답을 반환하는 객체. 호출 여부는 검증하지 않음.
...
라인 커버리지는 측정이 간단하고 직관적이지만, 위의 `true` 입력처럼 모든 줄을 실행해도 조건의 반대 분기는 확인하지 않을 수 있다. 이 차이를 보기 위해 분기 커버리지를 함께 살펴본다.
...
- "높은 커버리지가 품질 좋은 테스트를 보장하지 않음.
- 하지만 낮은 커버리지는 테스트가 부족하다는 것을 보장함."
...
AAA 패턴은 준비(Arrange), 실행(Act), 검증(Assert)의 순서로 테스트를 구성한다. 먼저 필요한 객체와 입력을 만들고 테스트 대상을 실행한 뒤, 그 결과가 기대와 일치하는지 확인한다. 아래 할인 계산 테스트도 이 순서를 따른다.
