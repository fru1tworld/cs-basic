...
- 통합 테스트는 여러 컴포넌트가 함께 동작할 때 올바르게 작동하는지 검증하는 테스트임. 단위 테스트와 달리 실제 의존성(데이터베이스, 외부 API, 메시지 큐 등)을 사용함.

### 단위 테스트 vs 통합 테스트

예를 들어 UserService의 단위 테스트에서는 Repository를 Mock으로 대체해 서비스 동작에 집중한다. 여기서 다루는 통합 테스트는 실제 UserRepository와 PostgreSQL까지 연결해, 서비스가 저장소와 함께 동작하는지 확인한다.

### 통합 테스트의 범위

테스트 범위는 Controller에서 Service와 Repository로 이어지는 호출 경로까지 넓힐 수 있다. 이 경로에서 접근하는 외부 API와 DB도 함께 고려한다.

## 테스트 컨테이너 (Testcontainers)

### 개념

실제 의존성을 사용하는 테스트에는 데이터베이스 같은 실행 환경이 필요하다. Testcontainers는 Docker 컨테이너를 코드로 관리해 이러한 환경을 테스트에서 사용할 수 있게 하는 라이브러리다.

### 장점

- 프로덕션 환경과 동일한 기술 스택 사용
  - H2 대신 실제 PostgreSQL
...
다음 예제는 PostgreSQL 컨테이너를 시작하고 연결 정보를 Spring에 주입한다. 테스트마다 사용자 데이터를 지운 뒤 실제 저장과 조회 결과를 확인한다.
...
Test A가 만든 데이터를 정리하지 않으면 Test B는 예상하지 못한 데이터를 읽고 실패할 수 있다. Test A가 끝난 뒤 상태를 정리해 Test B가 깨끗한 상태에서 시작하게 해야 실행 순서에 따른 영향을 줄일 수 있다. 아래는 이를 위한 롤백, 테이블 정리, 독립 스키마, 컨테이너 재생성 방식이다.
