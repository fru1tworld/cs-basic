...
Jenkins Pipeline은 CI/CD 파이프라인을 `Jenkinsfile`이라는 텍스트 파일로 정의하는 기능이다. 소스 코드와 함께 버전 관리하고 리뷰할 수 있어 빌드와 배포 절차의 변경도 추적할 수 있다.

Pipeline에서는 Agent로 실행 환경(`any`, `docker`, `kubernetes`, `label`)을 정하고, Stages 아래 Build, Test, Analyze, Deploy 같은 단계를 배치한다. 각 단계의 실제 작업은 Steps에 작성하며, Post에는 `always`, `success`, `failure`, `unstable`, `changed` 조건에 따른 후처리를 둔다.

### Pipeline의 이점

- 이점: 코드로 관리 (Pipeline as Code):
  - 설명: Jenkinsfile을 Git으로 버전 관리, 코드 리뷰 가능
- 이점: 재사용성:
  - 설명: Shared Library로 공통 로직 재사용
- 이점: 내구성:
  - 설명: Jenkins 재시작 후에도 파이프라인 상태 유지
- 이점: 일시 중지 가능:
  - 설명: 수동 승인 대기 등 중간 상태에서 일시 중지
- 이점: 확장성:
  - 설명: 다양한 플러그인과 통합
...
- 가능 신규 프로젝트
- 가능 표준적인 CI/CD 워크플로우
- 가능 팀에 Jenkins 전문가가 없는 경우
- 가능 코드 리뷰 및 유지보수 용이성 중요
- 가능 Blue Ocean UI 사용
...
- Shared Library를 사용하면 여러 파이프라인에서 공통 로직을 재사용할 수 있음.

- # Shared Library 디렉터리 구조
...
Jenkins로 빌드한 결과물을 배포할 때, 원하는 배포 상태를 Git에 기록하고 실제 환경과 맞추는 방식을 사용할 수 있다. GitOps는 이렇게 Git을 기준(Single Source of Truth)으로 인프라와 애플리케이션을 선언적으로 관리한다.
...
ArgoCD는 Poll이나 Webhook으로 Git 저장소의 변경을 확인한다. 저장소에는 YAML, Helm, Kustomize 형태의 애플리케이션 매니페스트를 둔다.

Repo Server는 저장소와 통신하며 매니페스트를 생성하고 캐시한다. Application Controller는 이 원하는 상태를 대상 Kubernetes 클러스터의 실제 상태와 비교하고 동기화하는 조정 루프(Reconciliation Loop)를 실행한다. API Server는 gRPC/REST로 Web UI, CLI, CI/CD 도구가 이 기능에 접근하는 인터페이스를 제공한다.
...
- 주의 주의: 평문 Secret을 Git에 커밋하지 마세요!
...
두 도구를 연결할 때는 애플리케이션 소스 저장소와 매니페스트 저장소를 나눌 수 있다. Jenkins는 소스를 빌드하고 테스트한 뒤 이미지를 푸시하고, 배포할 이미지 버전을 매니페스트 저장소에 반영한다.

ArgoCD는 Git Poll로 이 변경을 감지하고 Kubernetes 클러스터를 동기화한다. 이 구성에서 Jenkins는 빌드·테스트·이미지 푸시의 CI를, ArgoCD는 배포·상태 관리·롤백의 CD를 맡는다.

### Jenkinsfile: Manifest 업데이트
