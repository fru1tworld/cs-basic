...
Blue-Green Deployment는 동일한 프로덕션 환경 두 개를 유지하고, 사용하지 않는 환경에 새 버전을 배포한 뒤 트래픽을 전환하는 전략이다. 초기에는 로드 밸런서가 Blue의 v1.0.0으로 요청을 보내고, 같은 버전의 Green은 비활성 상태로 둔다.

Blue가 트래픽을 처리하는 동안 Green에 v1.1.0을 배포하고 테스트한다. 준비가 끝나면 요청을 Green으로 전환하고, Blue의 v1.0.0은 롤백에 사용할 수 있도록 대기시킨다.

### 장점과 단점

- 구분: 장점:
  - 내용: - Zero Downtime: 트래픽 전환 시 다운타임 없음
- 구분: 항목:
  - 내용: - 즉시 롤백: 문제 발생 시 이전 환경으로 즉시 전환
- 구분: 항목:
  - 내용: - 완전한 테스트: 프로덕션과 동일한 환경에서 테스트 가능
- 구분: 항목:
  - 내용: - 간단한 트래픽 관리: 100% 전환으로 복잡한 라우팅 불필요
- 구분: 단점:
  - 내용: - 비용: 두 배의 인프라 리소스 필요
- 구분: 항목:
  - 내용: - DB 동기화: 데이터베이스 스키마 변경 시 복잡성 증가
- 구분: 항목:
  - 내용: - 세션 관리: Stateful 애플리케이션에서 세션 유실 가능
...
Canary Deployment는 새 버전을 일부 사용자, 예를 들어 5%에게 먼저 제공한다. 실제 트래픽의 오류율과 지연시간 등을 관찰해 문제가 없는지 확인한 뒤 점진적으로 전체 사용자에게 확대한다.
...
- 구분: 장점:
  - 내용: - 위험 최소화: 일부 사용자에게만 영향
- 구분: 항목:
  - 내용: - 실제 트래픽 테스트: 프로덕션 환경에서 검증
- 구분: 항목:
  - 내용: - 점진적 롤아웃: 문제 발생 시 영향 범위 제한
- 구분: 항목:
  - 내용: - 비용 효율적: Blue-Green 대비 적은 리소스
- 구분: 단점:
  - 내용: - 복잡한 모니터링: 세밀한 관찰 필요
- 구분: 항목:
  - 내용: - 트래픽 분산 복잡성: 정교한 라우팅 구현 필요
- 구분: 항목:
  - 내용: - 롤백 시간: 즉각적인 롤백이 어려울 수 있음

### Argo Rollouts를 활용한 Canary 구현
...
- Rolling Update는 기존 인스턴스를 점진적으로 새 버전으로 교체하는 배포 전략임. Kubernetes의 기본 배포 전략이며, 추가 인프라 없이 무중단 배포가 가능함.
...
- 구분: 장점:
  - 내용: - 추가 인프라 불필요: 기존 리소스 내에서 배포
- 구분: 항목:
  - 내용: - 무중단 배포: 점진적 교체로 서비스 연속성 유지
- 구분: 항목:
  - 내용: - Kubernetes 기본 지원: 별도 설정 없이 사용 가능
- 구분: 항목:
  - 내용: - 리소스 효율적: Blue-Green 대비 저비용
- 구분: 단점:
  - 내용: - 배포 시간: 대규모 환경에서 시간 소요
- 구분: 항목:
  - 내용: - 버전 혼재: 배포 중 두 버전이 동시 운영
- 구분: 항목:
  - 내용: - 롤백 복잡: 중간 실패 시 롤백에 시간 소요
- 구분: 항목:
  - 내용: - 호환성 필수: 신/구 버전 간 호환성 필요
...
- A/B Testing은 두 가지(또는 그 이상) 버전의 기능을 서로 다른 사용자 그룹에게 제공하여 성능, 사용성, 전환율 등을 비교 분석하는 실험 기법임.

라우터는 쿠키, 헤더, 사용자 ID 등으로 사용자를 나눠 Version A(Control)와 Version B(Variant)에 보낸다. 예를 들어 A에는 기존 파란 버튼을, B에는 새 초록 버튼을 표시하고 분석 플랫폼에서 전환율, 클릭률, 이탈률, 사용자당 매출을 비교한다.

일부 사용자에게 다른 버전을 보여준다는 점은 Canary와 비슷하지만 확인하려는 지표가 다르다. Canary가 오류율과 지연시간 같은 기술적 안정성을 확인한다면, A/B 테스트는 전환율과 매출 같은 비즈니스 성과를 비교한다.

### A/B Testing vs Canary Deployment

- 구분: 목적:
  - A/B Testing: 비즈니스 지표 최적화
  - Canary Deployment: 기술적 안정성 검증
- 구분: 분할 기준:
  - A/B Testing: 사용자 세그먼트
  - Canary Deployment: 트래픽 비율
- 구분: 측정 지표:
  - A/B Testing: 전환율, CTR, 매출
  - Canary Deployment: 에러율, 레이턴시, 리소스
- 구분: 기간:
  - A/B Testing: 통계적 유의성 확보까지
  - Canary Deployment: 안정성 확인까지
- 구분: 롤백:
  - A/B Testing: 실험 종료 (낮은 성과 버전 제거)
  - Canary Deployment: 문제 감지 시 즉시 롤백

### Istio를 활용한 A/B Testing
...
트래픽을 다른 버전으로 보내는 대신 같은 코드 안에서 기능 노출을 제어할 수도 있다. Feature Flag(기능 플래그)는 런타임에 기능을 켜고 끄는 방식으로 코드 배포와 기능 릴리스를 분리한다.

- Feature Flag 활용 패턴
- Release Toggle (릴리스 토글)
- if (featureFlags.isEnabled("new-checkout")) {
- return newCheckoutFlow();
- } else {
- return legacyCheckoutFlow();
- 용도: 미완성 기능 숨기기, 점진적 롤아웃
- Experiment Toggle (실험 토글)
...
- 도구: LaunchDarkly:
  - 타입: SaaS
  - 특징: 실시간 업데이트, 상세 타겟팅, A/B 테스트
- 도구: Unleash:
  - 타입: 오픈소스/SaaS
  - 특징: 셀프 호스팅 가능, 활성화 전략
- 도구: Flagsmith:
  - 타입: 오픈소스/SaaS
  - 특징: Remote Config 지원, 세그먼트
- 도구: ConfigCat:
  - 타입: SaaS
  - 특징: 간단한 설정, 빠른 시작
- 도구: Split:
  - 타입: SaaS
  - 특징: Feature Experimentation, 통계 분석
...
- 롤백 전략 비교
- 즉시 롤백 (Instant Rollback)
- v1.1.0, →, v1.0.0,  Blue-Green 전환
- (문제), 즉시, (안정),  트래픽 즉시 이동
- 다운타임: 수초
- 점진적 롤백 (Gradual Rollback)
- v1.1.0, →, v1.0.0,  Canary 역방향
- 90%, →, 100%,  점진적 트래픽 이동
- 영향 최소화
- 재배포 롤백 (Redeploy Rollback)
- v1.1.0, →, v1.0.0,  이전 버전 재빌드/배포
- (삭제), 배포, (배포),  시간 소요 (분 단위)
- Rolling Update 방식
- 기능 롤백 (Feature Rollback)
