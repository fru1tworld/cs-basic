...
GitHub Actions Workflow는 `.github/workflows/` 아래 YAML 파일로 정의한다. `on`으로 실행 계기를 지정하고, `jobs` 안에 실행 환경과 작업 단계를 구성한다. 다음 예제는 push와 pull request 이벤트뿐 아니라 배포 환경을 선택하는 수동 실행도 정의한다.
...
- 재사용 가능한 Workflow (Reusable Workflows) 는 다른 Workflow에서 호출할 수 있는 템플릿 Workflow임. 2025년 기준으로 최대 10단계 중첩과 50개의 Workflow 호출이 가능함.

Reusable Workflow는 `workflow_call`로 전체 Job 구성을 재사용한다. 여러 Job과 각 Step을 묶은 파이프라인 템플릿이며, Job에서 호출하고 독립적인 실행 환경을 지정한다.

Composite Action은 `action.yml`에 Step 집합을 정의하는 작업 템플릿이다. 호출하는 Workflow의 Step에서 사용하고 그 실행 환경을 공유한다. 따라서 파이프라인 전체를 공유할지, 한 Job 안의 공통 작업을 공유할지에 따라 선택한다.
...
- Self-hosted Runner는 직접 호스팅하는 GitHub Actions 실행 환경으로, GitHub-hosted Runner와 달리 하드웨어, OS, 소프트웨어를 자유롭게 구성할 수 있음.
...
### Kubernetes에서 Self-hosted Runner (ARC)

- Actions Runner Controller (ARC) 를 사용하여 Kubernetes에서 Self-hosted Runner를 동적으로 관리할 수 있음.
...
- Private Repository 전용
- 주의 Public Repository에서 Self-hosted Runner 사용 금지!
...
Secrets는 조직, 저장소, 환경 단위로 관리한다. Organization Secrets는 조직에서 중앙 관리하며 접근 가능한 저장소를 제한할 수 있다. Repository Secrets는 저장소 관리자가 설정하고 해당 저장소에서만 사용하며, 같은 이름의 조직 Secret을 덮어쓸 수 있다.

Environment Secrets는 production이나 staging 같은 특정 환경에서 사용하고 승인 워크플로우와 연결할 수 있다. 같은 이름이 겹치면 `Environment > Repository > Organization` 순서로 우선한다.
...
같은 테스트를 OS와 언어 버전별로 반복해야 한다면 Matrix Build로 조합을 정의할 수 있다. 아래 예제는 세 OS와 Node.js 18, 20, 22를 조합하고, `include`와 `exclude`로 추가 설정과 제외할 환경을 지정한다. 각 조합은 같은 작업을 병렬로 실행한다.
...
- [Jenkins ArgoCD](04-Jenkins-ArgoCD.md)
