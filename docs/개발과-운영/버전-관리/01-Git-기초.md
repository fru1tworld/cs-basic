...
Git이 파일과 커밋을 저장하는 구조를 살펴보고, 그 구조를 바탕으로 reset, rebase, cherry-pick 같은 명령이 무엇을 바꾸는지 정리한다.

## Git 내부 구조

- Git은 단순한 버전 관리 시스템이 아닌 Content-Addressable Filesystem임. 모든 데이터를 SHA-1 해시를 키로 사용하여 저장함.

### Git의 4가지 객체 타입

Commit 객체는 부모 Commit과 프로젝트의 Tree를 참조한다. Tree는 `file1`, `file2`의 내용을 담은 Blob과 하위 디렉터리 `dir`의 Tree를 연결하고, 그 하위 Tree는 다시 `file3`의 Blob을 참조한다. 파일 내용과 이름, 디렉터리 구조를 이처럼 서로 다른 객체로 나눠 저장한다.

#### Blob (Binary Large Object)

- 파일의 내용만을 저장함. 파일명이나 경로 정보는 포함하지 않음.
...
내용이 같으면 해시도 같으므로 파일명이 달라도 같은 Blob을 참조한다. 같은 내용을 중복 저장하지 않는 이 특성이 저장 공간을 줄여 준다.

#### Tree

Blob에 없는 파일명과 권한, 디렉터리 구조는 Tree가 맡는다. Tree는 각 항목의 이름과 권한에 Blob 또는 하위 Tree 참조를 연결한다.
...
- 스냅샷 메타데이터를 저장함. Tree, 부모 커밋, 작성자, 커미터, 메시지를 포함함.
...
- 중요: Git은 델타(차이점)가 아닌 전체 스냅샷을 저장함. SVN, CVS와 달리 각 커밋은 완전한 프로젝트 상태를 참조함.

#### Tag

- 커밋을 가리키는 영구적인 참조임. Annotated Tag는 별도의 객체로 저장됨.
...
Git은 개별 파일로 저장한 Loose 객체를 Packfile로 묶어 압축할 수 있다. 다음 명령은 이 정리를 수동으로 실행하고 생성된 Packfile과 인덱스를 확인한다.
...
Packfile은 유사한 객체 사이의 차이를 델타 압축으로 저장한다. 큰 파일의 일부만 바뀌면 그 차이를 저장할 수 있어 저장 공간과 네트워크 전송량을 줄인다.
...
아래 예제에서 main에는 A, B, C가 있고 feature에는 D, E가 있다. Merge는 두 이력을 합치는 M 커밋을 만들어 분기 관계를 남긴다. Rebase는 feature의 변경을 main 뒤에 다시 적용해 D', E'를 만들므로 이력이 선형으로 이어진다.
...
- 구분: 충돌 해결:
  - Merge: 한 번에 해결
  - Rebase: 커밋마다 해결 필요할 수 있음
- 구분: bisect:
  - Merge: 복잡해질 수 있음
  - Rebase: 용이함
- 구분: 되돌리기:
  - Merge: merge 커밋 revert로 쉬움
  - Rebase: 복잡할 수 있음
...
- 파일별 속성을 정의함. Line ending, diff, merge 전략 등을 설정함.
