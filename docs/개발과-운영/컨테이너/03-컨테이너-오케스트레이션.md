...
새 Pod를 실행하려면 먼저 이를 수용할 노드를 골라야 한다. kube-scheduler는 스케줄링을 기다리는 Pod를 확인하고, 실행 조건을 만족하는 노드 중에서 배치할 곳을 결정한다.

- Scheduling Process
- Watch: API Server에서 스케줄 대기 Pod 감시
- Filtering (Predicates): 실행 가능한 노드 필터링
- 리소스 충분한가? (CPU, Memory)
- Node Selector 만족하는가?
- Affinity/Anti-Affinity 조건 만족하는가?
- Taint/Toleration 충족하는가?
- Pod가 요청한 볼륨 마운트 가능한가?
- Scoring (Priorities): 최적 노드에 점수 부여
- 리소스 균형 (LeastRequestedPriority)
- Pod 분산 (SelectorSpreadPriority)
- 선호도 가중치 (NodeAffinityPriority)
- 이미지 로컬리티 (ImageLocalityPriority)
- Binding: 선택된 노드에 Pod 할당
...
- nodeSelector보다 유연한 노드 선택 규칙을 정의함.
...
Pod의 IP는 동적으로 바뀌므로 호출하는 쪽에서 주소를 고정해 두기 어렵다. 서비스 디스커버리는 이름이나 API를 통해 현재 연결할 대상을 찾게 해 준다.
...
- CoreDNS는 Kubernetes의 기본 DNS 서버임.
...
Recreate는 실행 중인 v1 Pod 3개를 모두 종료한 뒤 v2 Pod 3개를 시작한다. 두 버전이 동시에 실행되지 않는 대신, 기존 Pod가 종료되고 새 Pod가 준비될 때까지 서비스 중단이 발생한다.
...
자동 확장은 부족한 자원이 어느 수준에 있는지에 따라 접근이 달라진다. Pod 수를 늘릴 수도 있고, 각 Pod의 자원을 늘리거나 이를 수용할 노드를 추가할 수도 있다.

| 방식 | 조절 대상 | 적용 상황과 특징 |
| --- | --- | --- |
| HPA (Horizontal Pod Autoscaler) | Pod 수 | 수평 확장으로, Stateless 애플리케이션에 적합하다. |
| VPA (Vertical Pod Autoscaler) | Pod의 CPU와 메모리 | 수직 확장으로, Stateful 또는 단일 인스턴스에 적합하다. |
| Cluster Autoscaler | 노드 수 | 클라우드 환경에서 동작하며 HPA와 함께 사용한다. |
| KEDA (Kubernetes Event Driven Autoscaler) | 이벤트에 따른 스케일링 | Kafka, RabbitMQ 등의 외부 메트릭을 지원하고 0까지 축소할 수 있다. |
...
- 가능 대규모 프로덕션 환경
- 가능 복잡한 마이크로서비스 아키텍처
- 가능 고급 스케줄링/오토스케일링 필요
- 가능 멀티 클라우드/하이브리드 클라우드
- 가능 활발한 에코시스템 활용 (Istio, Argo 등)
- 가능 전담 DevOps/SRE 팀 보유
- Docker Swarm 선택:
- 가능 소규모 팀/프로젝트
- 가능 빠른 셋업 필요
- 가능 Docker Compose에 익숙
- 가능 기본적인 오케스트레이션만 필요
- 가능 엣지 컴퓨팅/IoT 환경
- 가능 학습/테스트 목적
- 2025년 현실:
