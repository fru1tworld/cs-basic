...
- Microsoft Azure는 전 세계에서 두 번째로 큰 클라우드 서비스 제공업체로, 약 24%의 글로벌 시장 점유율을 보유하고 있음. 특히 엔터프라이즈 시장에서 강력한 입지를 가지고 있으며, Microsoft 365, Dynamics 365, Power Platform과의 통합이 강점임.
...
Azure 리소스는 테넌트, 선택적인 관리 그룹(Management Group), 구독(Subscription), 리소스 그룹(Resource Group)의 계층으로 관리한다. 예를 들어 리소스 그룹 A에 Virtual Machine, Storage Account, Virtual Network를 묶고, 그룹 B에 App Service와 SQL Database를 둘 수 있다.

모든 리소스는 리소스 그룹에 속해야 한다. 리소스 그룹 자체에는 리전이 지정되지만, 그 안의 리소스는 다른 리전에 배치할 수 있다. RBAC 권한은 이 계층을 따라 상속되므로 역할을 할당할 범위를 함께 고려해야 한다.

## Azure Virtual Machines

Azure VM은 운영 체제와 서버 구성을 선택해 사용하는 IaaS 컴퓨팅 서비스다. 개발용 소규모 서버부터 GPU 연산이나 대규모 메모리가 필요한 작업까지, 워크로드에 맞는 VM 시리즈를 선택할 수 있다.
...
- 시리즈: D-series:
  - 용도: 범용
  - 사용 사례: 엔터프라이즈 앱, 중간 규모 DB
- 시리즈: E-series:
  - 용도: 메모리 최적화
  - 사용 사례: 인메모리 DB, SAP HANA
- 시리즈: F-series:
  - 용도: 컴퓨팅 최적화
  - 사용 사례: 배치 처리, 게임 서버
- 시리즈: N-series:
...
- 시리즈: L-series:
  - 용도: 스토리지 최적화
  - 사용 사례: 대용량 로컬 SSD, NoSQL
- 시리즈: M-series:
  - 용도: 대규모 메모리
  - 사용 사례: SAP HANA (최대 12TB RAM)
...
AKS는 Azure에서 Kubernetes 클러스터를 운영할 수 있게 해 주는 완전 관리형 서비스다. 클러스터를 구성할 때는 노드 풀과 네트워크뿐 아니라 Azure의 ID 관리, 컨테이너 레지스트리, 모니터링 서비스를 어떻게 연결할지도 정한다.
...
Cosmos DB는 여러 리전에 데이터를 분산하는 다중 모델 데이터베이스 서비스다. 사용하는 API와 처리량 방식에 더해 일관성 수준도 선택할 수 있어, 응답 시간과 데이터 일관성 요구를 함께 고려해야 한다.
...
Azure Functions는 HTTP 요청, 데이터 변경, 메시지, 타이머 등의 이벤트에 맞춰 함수를 실행한다. 같은 함수라도 호스팅 플랜에 따라 비용, 네트워크 통합, 시작 지연 등의 조건이 달라지므로 실행 방식에 맞는 플랜을 선택한다.
...
- Azure DevOps는 완전한 DevOps 도구 체인을 제공하는 서비스임.

### Azure DevOps 구성 요소

Azure DevOps는 계획부터 코드 관리, 검증, 배포까지 이어지는 도구를 제공한다. Boards에서 작업 항목과 스프린트·칸반 보드를 관리하고, Repos에서 Git 저장소와 브랜치 정책, Pull Request를 운영한다. Pipelines는 YAML 기반 CI/CD를 여러 플랫폼에서 실행한다.

테스트 관리는 Test Plans로, npm·NuGet·Maven·pip 패키지 관리는 Artifacts로 연결할 수 있다. 아래 파이프라인 예시는 빌드와 테스트를 마친 뒤 브랜치에 따라 스테이징 또는 프로덕션에 배포한다.
...
- Azure AD(현재 Microsoft Entra ID)는 Microsoft의 클라우드 기반 ID 및 액세스 관리 서비스임.
...
관리형 ID는 리소스와 수명 주기를 함께할지, 여러 리소스에서 재사용할지에 따라 두 종류로 나뉜다.

| 종류 | 생성과 수명 주기 | 공유 여부 |
| --- | --- | --- |
| 시스템 할당(System-Assigned) | 리소스와 1:1로 연결되어 설정이 간단하며, 리소스 삭제 시 함께 삭제된다. | 공유할 수 없다. |
| 사용자 할당(User-Assigned) | 독립적으로 생성하고 수동으로 삭제한다. | 여러 리소스에서 재사용할 수 있다. |

이 ID로 Key Vault의 비밀, Storage Account, Azure SQL, Azure Service Bus와 Event Hub에 접근할 수 있다. 아래 코드는 자격 증명을 얻은 뒤 각 서비스의 클라이언트에 전달하는 흐름이다.
