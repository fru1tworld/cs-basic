...
- AWS(Amazon Web Services)는 전 세계에서 가장 큰 클라우드 서비스 제공업체로, 약 31%의 글로벌 시장 점유율을 보유하고 있음. 200개 이상의 완전 관리형 서비스를 제공하며, 컴퓨팅, 스토리지, 데이터베이스, 네트워킹, 머신러닝 등 다양한 영역을 포괄함.

## AWS의 글로벌 인프라

AWS의 리전(Region)은 여러 가용 영역(Availability Zone)으로 구성되고, 가용 영역 안에는 데이터 센터가 있다. 예를 들어 한 리전에 가용 영역 1, 2, 3이 있고 각각 데이터 센터 A·B, C·D, E를 두는 구조다.

CloudFront CDN은 별도의 엣지 로케이션(Edge Location)을 통해 콘텐츠를 전달한다. 리전과 가용 영역이 서비스를 실행할 위치라면, 엣지 로케이션은 사용자에게 콘텐츠를 전달하는 지점으로 볼 수 있다.

## 핵심 서비스

### Amazon EC2 (Elastic Compute Cloud)

EC2는 가상 서버를 인스턴스 단위로 제공하는 컴퓨팅 서비스다. 실행할 워크로드에 맞춰 CPU, 메모리, GPU, 스토리지에 특화된 인스턴스 유형을 선택하고, 사용 기간과 중단 허용 여부에 따라 구매 방식을 정한다.

#### 인스턴스 유형

- 패밀리: t3/t4g:
  - 용도: 범용 (버스트 가능)
  - 사용 사례: 개발/테스트, 소규모 웹 서버
- 패밀리: m6i/m7i:
  - 용도: 범용 (고정 성능)
  - 사용 사례: 프로덕션 웹 서버, 애플리케이션 서버
- 패밀리: c6i/c7i:
  - 용도: 컴퓨팅 최적화
  - 사용 사례: 배치 처리, 게임 서버, 과학 연산
- 패밀리: r6i/r7i:
  - 용도: 메모리 최적화
  - 사용 사례: 인메모리 캐시, 대용량 DB
- 패밀리: p4d/p5:
  - 용도: GPU 최적화
  - 사용 사례: 머신러닝, 영상 처리
- 패밀리: i4i:
  - 용도: 스토리지 최적화
  - 사용 사례: NoSQL DB, 데이터 웨어하우스

#### 구매 옵션

| 옵션 | 할인율 | 약정 기간 | 특징 |
| --- | --- | --- | --- |
| On-Demand | 0% | 없음 | 사용 기간을 유연하게 정할 수 있다. |
| Reserved | 최대 72% | 1~3년 | 예측 가능한 워크로드에 사용한다. |
| Spot | 최대 90% | 없음 | 실행 중단을 허용하는 워크로드에 사용한다. |
| Savings Plan | 최대 72% | 1~3년 | 유연한 할인 방식을 제공한다. |
...
- S3는 무제한 확장이 가능한 객체 스토리지 서비스임. 내구성 99.999999999% (11 9's)를 보장함.
...
Lambda는 코드를 업로드하면 실행 환경을 관리해 주는 서버리스 컴퓨팅 서비스다. 서버를 직접 운영하는 부담은 줄어들지만, 함수의 실행 시간과 메모리, 패키지 크기 같은 제한 안에서 코드를 구성해야 한다.
...
- 항목: 최대 실행 시간:
  - 제한/특성: 15분
- 항목: 메모리:
  - 제한/특성: 128MB ~ 10,240MB
- 항목: 임시 스토리지:
  - 제한/특성: 최대 10GB (/tmp)
- 항목: 동시 실행:
  - 제한/특성: 기본 1,000 (조정 가능)
- 항목: 배포 패키지 크기:
  - 제한/특성: 50MB (압축), 250MB (압축 해제)
- 항목: 컨테이너 이미지:
  - 제한/특성: 최대 10GB
...
- VPC는 AWS 클라우드에서 논리적으로 격리된 네트워크 환경을 제공함.

- \*자세한 내용은 [AWS 네트워킹](#aws-네트워킹) 섹션 참조\*

### Amazon ECS (Elastic Container Service)

- ECS는 Docker 컨테이너를 대규모로 실행하고 관리하기 위한 완전 관리형 서비스임.

#### ECS 아키텍처

ECS Cluster 안에서 Service가 Task를 관리한다. 각 Task에는 애플리케이션 컨테이너와 로깅·모니터링용 Sidecar를 함께 넣을 수 있다. 실행 환경으로는 EC2 또는 서버리스 방식의 Fargate를 선택한다.
...
- IAM은 AWS 리소스에 대한 접근을 안전하게 제어하는 서비스임.
...
#### Service Control Policies (SCP) - 조직 수준 가드레일
...
`10.0.0.0/16` VPC를 가용 영역 A와 B에 나누고, 각 영역에 공개 진입점, 애플리케이션, 데이터베이스용 서브넷을 구분하는 예시다. VPC에는 Internet Gateway(IGW)를 연결한다.

| 계층 | 가용 영역 A | 가용 영역 B | 배치할 리소스 |
| --- | --- | --- | --- |
| Public | `10.0.1.0/24` | `10.0.2.0/24` | ALB, NAT Gateway |
| Private App | `10.0.3.0/24` | `10.0.4.0/24` | 애플리케이션 EC2 |
| Private DB | `10.0.5.0/24` | `10.0.6.0/24` | RDS Primary와 Standby |

데이터베이스 계층의 Primary와 Standby는 가용 영역을 나눠 배치하고 동기 복제를 구성한다.
...
- Private Subnet의 NACL 규칙 예시임.
- 인바운드 규칙:
  - 100: Custom, 포트 8080, 출발지 `10.0.1.0/24`, Allow
  - 110: Custom, 포트 8080, 출발지 `10.0.2.0/24`, Allow
  - 120: Custom, 포트 1024~65535, 출발지 `0.0.0.0/0`, Allow, 응답 트래픽용
  - 기본 규칙 `*`: 모든 유형과 포트, 출발지 `0.0.0.0/0`, Deny
- 아웃바운드 규칙:
  - 100: HTTPS, 포트 443, 목적지 `0.0.0.0/0`, Allow
  - 110: Custom, 포트 3306, 목적지 `10.0.5.0/24`, Allow, 데이터베이스 접근용
  - 120: Custom, 포트 1024~65535, 목적지 `0.0.0.0/0`, Allow, 응답 트래픽용
  - 기본 규칙 `*`: 모든 유형과 포트, 목적지 `0.0.0.0/0`, Deny
...
Target Tracking은 지정한 메트릭이 목표값을 유지하도록 인스턴스 수를 조정하는 방식이다. 예를 들어 CPU 사용률 목표를 50%로 정했는데 부하가 늘어 80%에 이르면, 인스턴스를 추가해 부하를 분산하고 목표값에 가까워지도록 한다. 아래 Terraform 예시는 CPU 사용률과 ALB의 인스턴스당 요청 수를 각각 기준으로 설정한다.
