...
Google Cloud Platform(GCP)은 Google 검색, Gmail, YouTube 등에 쓰이는 인프라를 기반으로 제공하는 클라우드 플랫폼이다. 이 문서에서는 가상 머신과 컨테이너 실행 환경을 먼저 살펴보고, 데이터 분석·함수 실행·데이터베이스·네트워킹으로 이어서 정리한다.
...
Compute Engine은 가상 머신을 제공하는 IaaS(Infrastructure as a Service) 서비스다. 워크로드에 맞는 머신 패밀리를 선택할 수 있고, 아래 CLI 예시처럼 CPU와 메모리를 직접 지정하는 커스텀 머신 타입도 구성할 수 있다.

### 머신 유형

- 머신 패밀리: E2:
  - 용도: 비용 효율적
  - 사용 사례: 개발/테스트, 소규모 워크로드
- 머신 패밀리: N2/N2D:
  - 용도: 범용 균형
  - 사용 사례: 웹 서버, 앱 서버, 중간 규모 DB
- 머신 패밀리: C2/C2D:
  - 용도: 컴퓨팅 최적화
  - 사용 사례: 게임 서버, 과학 연산, HPC
- 머신 패밀리: M2/M3:
  - 용도: 메모리 최적화
  - 사용 사례: 대규모 인메모리 DB, SAP HANA
- 머신 패밀리: A2/A3:
  - 용도: 가속기 최적화
  - 사용 사례: ML 학습, 추론, HPC
- 머신 패밀리: T2D:
...
- GKE는 업계 최초의 완전 관리형 Kubernetes 서비스로, Google이 Kubernetes를 개발했기 때문에 가장 최신 기능을 빠르게 제공함.
...
GKE의 워크로드가 GCP 리소스에 접근하려면 사용할 서비스 계정을 연결해야 한다. 아래 예시는 Kubernetes Service Account(KSA)와 GCP Service Account(GSA)를 생성하고 IAM 바인딩과 어노테이션으로 연결한다.
...
Cloud Run은 컨테이너 실행 환경을 관리해 주는 서버리스 플랫폼이다. 아래 배포 예시처럼 소스 코드를 Buildpacks로 빌드해 올리거나, 준비된 컨테이너 이미지를 배포할 수 있다. 배포할 때 인스턴스 수, 메모리, CPU, 동시 요청 수와 네트워크 접근도 설정한다.
...
BigQuery는 페타바이트 규모의 데이터를 분석하는 완전 관리형 서버리스 데이터 웨어하우스다. 인프라를 직접 관리하지 않고 SQL로 데이터를 분석할 수 있으며, 아래 예시에서는 이벤트 테이블의 파티셔닝과 클러스터링부터 매출 집계와 이탈 예측까지 이어서 살펴본다.

### BigQuery 아키텍처

BigQuery는 실행 엔진, 저장소, 네트워크를 나눠 구성한다. Dremel 실행 엔진은 수천 개 노드에서 쿼리를 병렬 실행하며, 컬럼 기반 처리와 셔플 없는 조인 최적화를 사용한다.

저장은 분산 파일 시스템인 Colossus가 맡는다. 컬럼 지향 포맷인 Capacitor로 데이터를 저장하고 자동 복제와 암호화를 제공한다. 스토리지와 컴퓨팅은 분리되어 있으며, 이를 연결하는 Jupiter 네트워크는 1 Petabit/sec 대역폭을 제공한다.
...
Cloud Functions는 이벤트가 발생하면 함수를 실행하는 서버리스 플랫폼이다. HTTP 요청을 처리하는 함수뿐 아니라 Pub/Sub 메시지나 Firestore 변경에 반응하는 함수도 만들 수 있다. 아래 2세대 예시는 이 세 가지 진입점을 각각 구현한다.
...
- 배포 단위:
  ...
  - Cloud Run: 컨테이너
- 런타임:
  - Cloud Functions: 제한된 런타임
  - Cloud Run: 모든 컨테이너
- 실행 시간:
  - Cloud Functions: 최대 60분
  - Cloud Run: 최대 60분
- 메모리:
  - Cloud Functions: 최대 32GB
  - Cloud Run: 최대 32GB
- vCPU:
  - Cloud Functions: 최대 8
  - Cloud Run: 최대 8
- 동시성:
  - Cloud Functions: 1 요청/인스턴스
  - Cloud Run: 최대 1000/인스턴스
- 트리거:
  - Cloud Functions: HTTP, Events
  - Cloud Run: HTTP 중심
- 사용 사례:
  - Cloud Functions: 간단한 이벤트 처리
  - Cloud Run: 마이크로서비스
