...
Prometheus는 SoundCloud에서 개발한 오픈 소스 모니터링 시스템으로, CNCF 졸업 프로젝트다. 수집한 메트릭을 시계열 데이터베이스에 저장하고 PromQL로 조회한다. 여기에 Grafana를 연결하면 서비스별 대시보드를 만들고 지표의 변화를 시각화할 수 있다.

## Prometheus + Grafana 스택

Prometheus는 애플리케이션의 `/metrics`와 노드의 Exporter에서 메트릭을 가져와 TSDB에 저장한다. 저장한 데이터는 PromQL로 조회하고, 규칙을 평가해 알림을 발생시킨다.

Alertmanager는 알림의 라우팅, 그룹화, 억제, 발송을 맡아 Slack, Email, PagerDuty 등으로 전달한다. Grafana에서는 같은 데이터를 대시보드와 Explore로 살펴보고 알림도 구성할 수 있다.

## 2025년 Observability 트렌드

- 2025년 Observability 조사에 따르면, 70%의 기업이 Prometheus와 OpenTelemetry를 함께 사용하고 있으며, Grafana LGTM 스택(Loki, Grafana, Tempo, Mimir)이 벤더 종속성 없는 통합 관찰성 플랫폼으로 주목받고 있음.
...
여러 대시보드에서 복잡한 쿼리를 반복 실행한다면 계산 결과를 미리 저장할 수 있다. Recording Rule은 정해진 주기마다 쿼리를 평가해 새로운 시계열로 기록한다. 아래 예시는 에러율, p99 지연 시간, RPS, 가용성과 노드 자원 사용률을 미리 계산한다.
...
한 화면에 너무 많은 정보를 담으면 이상 징후를 찾기 어렵다. 이 문서에서는 3개 행, 행당 3개 패널, 패널당 3개 핵심 메트릭으로 시작하는 3-3-3 구성을 권장한다.

대시보드는 전체 상태에서 세부 원인으로 내려갈 수 있도록 나눈다. 개요(Overview)에서는 SLI/SLO와 핵심 비즈니스 지표를 보고, 서비스 화면에서는 RED 지표와 의존성, 주요 엔드포인트 상태를 확인한다. 더 좁혀 볼 필요가 있으면 상세(Detail) 화면에서 Pod·인스턴스, JVM·런타임, 데이터베이스 지표를 살핀다.
...
Prometheus가 `HighErrorRate` 규칙을 평가한다고 하자. `error_rate > 0.01` 조건이 참이면 PENDING 상태가 되고, `for: 5m` 동안 유지되면 FIRING 상태로 전환해 Alertmanager에 알린다.

Alertmanager는 같은 알림의 중복을 제거하고, 같은 서비스의 여러 인스턴스에서 발생한 알림을 묶는다. 그다음 레이블에 따라 `severity=critical`은 PagerDuty로, `severity=warning`은 Slack으로 보내는 식으로 수신자를 정한다. Email도 전송 대상으로 사용할 수 있다.

유지보수처럼 알림이 불필요한 동안에는 Silence를 설정한다. 클러스터 장애로 개별 Pod 알림까지 쏟아지는 경우에는 Inhibition 규칙으로 상위 장애에 따른 하위 알림을 억제할 수 있다.
...
수집 대상을 외부 시스템에서 파일로 관리한다면 File-based Service Discovery를 사용할 수 있다. 아래 설정은 JSON과 YAML 대상 파일을 30초마다 확인하며, 파일 안에 주소와 환경·팀 등의 레이블을 함께 적는다.
