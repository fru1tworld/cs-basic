...
도메인 주도 설계(DDD)는 복잡한 비즈니스 문제를 도메인 모델로 표현하고, 그 모델을 중심으로 소프트웨어를 설계하는 접근법이다. Eric Evans가 2003년에 제안했으며, 도메인 전문가와 개발자가 같은 용어로 규칙을 이해하는 데서 출발한다.

## DDD의 핵심 철학

- DDD의 핵심 철학
- "소프트웨어의 복잡성은 도메인 자체의 복잡성에서 비롯됨"
- Eric Evans
- DDD의 3가지 핵심
...
전략적 설계에서는 도메인을 어떤 영역으로 나눌지, 각 영역의 모델이 어디까지 유효한지 정한다. 먼저 Bounded Context로 모델의 경계를 살펴보고, Subdomain으로 비즈니스 영역을 구분한 뒤 Context Map으로 경계 사이의 관계를 연결한다.

### Bounded Context (경계 컨텍스트)

Bounded Context는 특정 도메인 모델이 유효한 범위다. 같은 "상품(Product)"이라도 카탈로그에서는 상세 정보와 마케팅 속성이 중요하고, 주문에서는 가격과 수량이, 배송에서는 무게와 취급 조건이 중요하다.

| 컨텍스트 | Product의 속성 | 관점 |
| --- | --- | --- |
| 상품 카탈로그 | id, name, description, images, category, attributes | 상세 정보와 마케팅 |
| 주문 관리 | id, name, price, quantity | 주문에 필요한 정보 |
| 배송 관리 | id, weight, dimensions, fragile, hazardous | 물류 정보 |

이 속성을 모두 하나의 모델에 모으기보다 각 Context에서 필요한 의미와 행동을 정의한다. 용어가 같아도 Context가 다르면 모델도 달라질 수 있다.

### Subdomain (서브도메인)

- 서브도메인은 비즈니스 도메인을 더 작은 영역으로 나눈 것임.
...
- Context Map은 여러 Bounded Context 간의 관계를 시각화한 것임.
...
경계를 정했다면 각 Bounded Context 안에서 모델을 구현해야 한다. 전술적 설계에서는 식별자로 구분하는 Entity, 값으로 구분하는 Value Object, 함께 변경하는 객체를 묶는 Aggregate 등을 사용한다.

### Entity (엔티티)

엔티티는 상태가 바뀌어도 같은 식별자(ID)로 구분하는 객체다. 아래 `Order`는 항목이나 상태가 바뀌어도 같은 주문이며, 동등성 역시 `id`로 비교한다.
...
Value Object는 별도 식별자 없이 속성 값으로 동등성을 판단하는 불변 객체다. 아래 `Money`는 금액과 통화로 비교하고, 덧셈이나 뺄셈을 하면 기존 값을 바꾸지 않고 새 객체를 반환한다. `Address.withCity`도 같은 방식으로 변경된 주소를 만든다.
...
- Aggregate는 데이터 변경의 단위로 취급되는 연관 객체들의 클러스터임.
...
Aggregate를 저장하고 다시 불러오는 계약은 Repository로 표현한다. 아래 인터페이스는 `Order` 단위로 조회·저장·삭제하는 기능을 정의하고, JPA 구현체가 도메인 객체와 저장용 엔티티를 변환한다.
...
어떤 규칙은 하나의 Entity나 Value Object에 배치하기 어렵다. 아래 가격 계산처럼 주문과 고객, 할인 정책을 함께 다루는 로직은 도메인 서비스로 표현할 수 있다.
...
- 유비쿼터스 언어(Ubiquitous Language)는 도메인 전문가와 개발자가 공유하는 공통 언어임.

- 유비쿼터스 언어
- "우리 도메인에서 사용하는 용어를 코드에 그대로 반영함"
- 불가 잘못된 예시 (기술 용어 사용):
...
- Domain Event는 도메인에서 발생한 중요한 비즈니스 사건을 나타냄.
...
- 감사 추적: 시스템에서 발생한 모든 변경 기록
- 이벤트 소싱: 이벤트로부터 현재 상태 재구성
- 통합: 다른 Bounded Context에 변경 알림
...
외부 시스템은 같은 개념도 다른 용어와 상태 값으로 표현할 수 있다. Anti-Corruption Layer(ACL)는 이 차이를 경계에서 변환해 외부 모델이 내부 도메인 전체로 퍼지지 않도록 한다. 아래 결제 예제는 Adapter가 외부 호출을 맡고 Translator가 응답을 내부 `PaymentResult`로 바꾸는 구조다.
...
- Event Storming은 도메인 이벤트를 중심으로 비즈니스 프로세스를 탐색하는 워크숍 기법임.

- Event Storming 프로세스
- 참여자: 도메인 전문가 + 개발자 + 디자이너
- 준비물: 넓은 벽, 포스트잇(여러 색상), 마커
- 포스트잇 색상 규칙
- 주황색: Domain Event (과거형, "~했음")
- 파란색: Command (명령, "~해라")
- 노란색: Aggregate (명사)
- 보라색: Policy/Process Manager ("~하면 ~함")
...
- [12 Factor Apps](04-12-Factor-Apps.md)
