...
GraphQL은 Facebook이 2012년에 개발하고 2015년에 오픈소스로 공개한 API 쿼리 언어다. 클라이언트가 필요한 필드를 쿼리에 명시하므로, 다음 예제처럼 사용자 정보와 게시물 제목을 함께 요청할 수 있다.
...
클라이언트가 요청할 수 있는 필드와 각 필드의 타입은 스키마로 정의한다. 이 스키마를 작성하는 언어가 SDL(Schema Definition Language)이다.
...
스키마가 요청할 수 있는 데이터의 구조를 정의한다면, Resolver는 각 필드의 데이터를 실제로 가져오는 함수다. 부모 필드의 결과와 요청 인수, 공유 컨텍스트를 받아 필요한 값을 반환한다.
...
게시물 목록을 한 번 조회하더라도 각 게시물의 저자를 별도로 조회하면 데이터베이스 호출은 한 번으로 끝나지 않는다. 다음 쿼리는 게시물 N개를 조회하는 1번의 쿼리에 저자 조회 N번이 더해지는 경우를 보여준다.
...
이때 저자 조회를 하나씩 실행하는 대신 모아서 처리할 수 있다. DataLoader는 요청을 배치(batch)하고 결과를 캐싱해 N+1 문제를 해결한다. 아래 예제에서는 사용자 ID를 모아 한 번에 조회한 뒤 입력 ID 순서에 맞춰 결과를 반환한다.
...
- 이벤트 루프 틱 내의 모든 .load() 호출 수집
- 다음 틱에서 batchLoadFn 한 번 호출
- 결과를 각 .load() 호출에 매핑
- Timeline:
...
앞의 REST 예제에서는 `/users/123`, `/users/123/posts`, `/users/123/followers`에 각각 요청하므로 총 3번의 요청이 필요하다. 각 응답에는 클라이언트가 쓰지 않는 필드도 포함될 수 있다.

GraphQL 예제에서는 `POST /graphql` 요청 하나에 `user(id: "123")`의 `name`, `posts { title }`, `followers { name }`을 함께 지정한다. 이 경우에는 한 번의 요청으로 필요한 필드만 받을 수 있다.
...
- 측면: 엔드포인트:
  ...
  - GraphQL: 단일 엔드포인트
- 측면: 데이터 페칭:
  - REST: Over/Under-fetching 발생
  - GraphQL: 정확한 데이터 페칭
- 측면: 버전 관리:
  - REST: URI 또는 헤더 버전
  - GraphQL: 스키마 진화 (additive)
- 측면: 캐싱:
  - REST: HTTP 캐싱 용이
  - GraphQL: 복잡한 캐싱 전략 필요
- 측면: 타입 시스템:
  - REST: 별도 문서/스키마 필요
  - GraphQL: 내장 스키마 시스템
- 측면: 에러 처리:
  - REST: HTTP 상태 코드
  - GraphQL: 항상 200, errors 필드
- 측면: 파일 업로드:
  - REST: 네이티브 지원
  - GraphQL: 별도 스펙 필요
- 측면: 실시간:
  - REST: 별도 구현 (WebSocket)
