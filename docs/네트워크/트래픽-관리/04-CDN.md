...
사용자가 원본 서버에서 멀리 떨어져 있으면 콘텐츠를 받는 데도 시간이 걸린다. CDN(Content Delivery Network)은 분산된 엣지 서버에서 콘텐츠를 제공해 이 지연과 원본 서버의 부하를 줄인다.
...
이 예제에서는 Origin Server 아래에 US-East, EU-West, AP-Tokyo의 Regional Edge Cache를 둔다. 그 아래 뉴욕, LA, 런던, 파리, 도쿄, 서울의 PoP(Point of Presence, 접속 지점)가 각 지역 사용자에게 콘텐츠를 제공한다.

### 요청 흐름

클라이언트는 DNS 조회로 CDN 접속 주소를 얻는다. 지역별 주소를 반환하거나 Anycast 주소로 네트워크 경로에 따라 엣지에 연결하는 방식이 있다.

엣지에 요청한 콘텐츠가 있으면 캐시에서 응답한다(Cache HIT). 없으면(Cache MISS) 오리진에서 가져와 캐시에 저장한 뒤 사용자에게 전달한다.
...
모든 엣지가 캐시 미스마다 오리진에 요청하면 같은 콘텐츠를 반복해서 가져올 수 있다. Tiered Caching은 L1인 Edge PoP에서 찾지 못한 콘텐츠를 L2 Regional Cache에서 다시 찾는다. 선택적으로 Origin Shield를 한 계층 더 두어 오리진까지 전달되는 요청을 줄일 수 있다.
...
정적 파일은 기존 캐시를 지우는 대신 URL을 바꾸는 방법도 있다. 아래 예제처럼 파일명에 버전이나 내용의 해시를 넣으면 새 파일은 새 URL로 요청하고, 이전 파일은 기존 캐시에 남겨 둘 수 있다.
...
한 상품이 상세 페이지와 카테고리 목록 등 여러 응답에 포함된다면 URL만으로 삭제 대상을 관리하기 번거롭다. 캐시 태그를 붙여 두면 상품이나 카테고리 변경 시 관련 응답을 한 그룹으로 무효화할 수 있다. 아래 코드는 `product-`, `category-`, `brand-` 태그를 함께 사용한다.
