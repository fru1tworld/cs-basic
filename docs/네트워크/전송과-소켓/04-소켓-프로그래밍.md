...
- 소켓(Socket)은 네트워크 통신의 끝점(endpoint)으로, 프로세스 간 통신(IPC)을 위한 인터페이스임. BSD 소켓 API는 1983년 BSD Unix에서 처음 도입되었으며, 현재 거의 모든 운영체제에서 표준으로 사용됨.
...
서버와 클라이언트 모두 `socket()`으로 소켓을 만든다. 서버는 `bind()`로 주소를 지정하고 `listen()`으로 연결을 받을 준비를 한다. 클라이언트가 `connect()`로 연결하면 서버는 `accept()`로 연결된 소켓을 얻는다. 이후 양쪽이 읽기와 쓰기로 데이터를 주고받고, 사용을 마치면 `close()`로 닫는다.
...
Linux에서는 SYN을 받고 SYN+ACK를 보낸 연결이 SYN Queue에서 마지막 ACK를 기다린다. 연결이 수립되면 Accept Queue에서 애플리케이션의 `accept()`를 기다린다. 여기서 `backlog`는 Accept Queue의 크기를 뜻하며, 실제 크기는 `min(backlog, /proc/sys/net/core/somaxconn)`으로 제한된다.
...
블로킹 소켓에서 `recv()`를 호출하면 데이터가 도착할 때까지 호출한 스레드가 기다린다. 이 대기 동안 CPU를 사용하지는 않지만, 같은 스레드가 다른 연결을 처리할 수도 없다. 연결마다 스레드를 두는 구조에서는 연결 수가 늘수록 스레드 생성과 문맥 교환 비용도 늘어난다.
...
논블로킹 소켓은 읽을 데이터가 없으면 `EAGAIN`이나 `EWOULDBLOCK`으로 즉시 반환한다. 호출자는 다른 작업을 하다가 다시 읽을 수 있다. 다만 읽기 시도만 계속 반복하면 Busy Waiting으로 CPU를 낭비하므로, 데이터가 준비되었을 때 알려 주는 이벤트 알림이 필요하다.
...
I/O 멀티플렉싱은 여러 소켓의 상태를 함께 기다리는 방식이다. `select`, `poll`, `epoll`이 작업 가능한 소켓을 알려 주면 이벤트 루프가 해당 소켓을 처리한다. 이를 통해 연결마다 스레드를 두지 않고도 여러 연결의 I/O를 진행할 수 있다.
...
- C10K(Concurrent 10,000) 문제는 1999년 Dan Kegel이 제기한 것으로, 단일 서버에서 10,000개의 동시 연결을 효율적으로 처리하는 것에 대한 도전 과제였음.

- C10K 문제의 핵심:
- 전통적인 방식 (스레드 per 연결):
- 10,000 연결 = 10,000 스레드
- 문제점:
- 메모리: 스레드당 ~1MB 스택 = 10GB 메모리
- 문맥 교환: 수만 번의 스위칭 오버헤드
- 스케줄러 부하: O(n) 스케줄링
- 스레드 생성 비용: 연결당 pthread_create 호출
- 해결책 (이벤트 기반):
...
아래 `asyncio` 서버는 클라이언트마다 코루틴을 만들고, 데이터를 기다리는 동안 이벤트 루프에 실행을 넘긴다. 읽은 데이터를 다시 쓰고 `drain()`으로 쓰기 버퍼의 흐름을 조절하는 에코 서버다.
...
- Client → HTTP 요청 1 → Server
- Server → HTTP 응답 1 → Client
- Client → TCP 종료 → Server
- Client → TCP 연결 → Server
- Client → HTTP 요청 2 → Server
- Server → HTTP 응답 2 → Client
- Client → TCP 종료 → Server
...
- Client → HTTP 요청 2 → Server
- Server → HTTP 응답 2 → Client
- Client → HTTP 요청 3 → Server
- Server → HTTP 응답 3 → Client
- Client → TCP 종료 (타임아웃 또는 명시적) → Server
- HTTP 헤더:
...
- WebSocket은 HTTP를 통해 연결을 시작한 후, 전이중(Full-Duplex) 양방향 통신을 제공하는 프로토콜임.
