...
네트워크 통신에는 신호 전송부터 주소 지정, 데이터 형식 변환까지 여러 기능이 필요하다. OSI(Open Systems Interconnection) 7계층 모델은 이 과정을 일곱 계층으로 나누어 각 기능의 역할을 설명한다. 국제표준화기구(ISO)가 1984년에 발표한 개념적 모델이다.

## OSI 모델의 목적

- 표준화: 서로 다른 벤더의 장비들이 상호 운용될 수 있도록 표준 제공
- 모듈화: 각 계층을 독립적으로 개발 및 업그레이드 가능
- 문제 해결: 네트워크 문제 발생 시 특정 계층으로 문제를 격리하여 디버깅 용이
- 교육: 네트워크 개념을 체계적으로 학습할 수 있는 프레임워크 제공

## OSI 7계층 상세

- OSI 7계층 모델
- Application, 사용자 인터페이스, 네트워크 서비스
- Presentation, 데이터 형식 변환, 암호화, 압축
- Session, 세션 관리, 동기화
- Transport, 종단간 신뢰성 있는 데이터 전송
- Network, 논리적 주소 지정, 라우팅
- Data Link, 물리적 주소 지정, 오류 검출
- Physical, 비트 전송, 물리적 연결
...
IPv4는 주소, TTL, 프로토콜, 단편화 정보로 라우팅과 전달을 지원한다. 각 필드의 순서와 비트 수는 [IPv4 패킷 구조](02-인터넷-작동-원리.md#패킷-스위칭-packet-switching)에서 살펴본다.
...
- Server → Session Acknowledgment → Client
- Client → Data Exchange → Server
- Server → Response → Client
- Client → Session Termination → Server
- Server → Termination ACK → Client
...
TCP/IP 4계층 모델은 OSI의 응용·표현·세션 계층을 응용 계층으로, 데이터 링크·물리 계층을 네트워크 접근 계층으로 묶어 설명한다. 전송 계층과 네트워크 계층은 각각 TCP/IP의 전송 계층과 인터넷 계층에 대응한다.
...
- 구분: 개발:
  - OSI 모델: ISO (국제표준화기구)
  - TCP/IP 모델: 미국 국방부 (DARPA)
- 구분: 목적:
  - OSI 모델: 이론적/개념적 표준
  - TCP/IP 모델: 실용적/구현 중심
- 구분: 프로토콜 의존성:
  - OSI 모델: 프로토콜 독립적
  - TCP/IP 모델: 특정 프로토콜에 의존
- 구분: 개발 시기:
  - OSI 모델: 1984년
  - TCP/IP 모델: 1970년대
- 구분: 사용:
...
- 계층: 7. Application:
  - 대표 프로토콜: HTTP, HTTPS, FTP, SMTP, DNS, SSH, Telnet, SNMP
  - 설명: 사용자 애플리케이션 서비스
- 계층: 6. Presentation:
  - 대표 프로토콜: SSL/TLS, JPEG, MPEG, ASCII, EBCDIC
  - 설명: 데이터 변환, 암호화
- 계층: 5. Session:
  - 대표 프로토콜: NetBIOS, RPC, PPTP, SIP
  - 설명: 세션 관리
- 계층: 4. Transport:
  - 대표 프로토콜: TCP, UDP, SCTP
  - 설명: 종단 간 전송
- 계층: 3. Network:
  - 대표 프로토콜: IP, ICMP, IGMP, ARP, RARP, OSPF, BGP, RIP
  - 설명: 라우팅, 주소 지정
- 계층: 2. Data Link:
  - 대표 프로토콜: Ethernet, PPP, HDLC, Frame Relay, ATM
  - 설명: 프레이밍, MAC
- 계층: 1. Physical:
  - 대표 프로토콜: RS-232, V.35, 100BASE-TX, IEEE 802.11
  - 설명: 물리적 전송
...
TCP 헤더에는 포트, 시퀀스 번호, ACK 번호, 제어 비트, 수신 윈도우와 체크섬이 들어가며 최소 20바이트를 차지한다. 옵션을 포함한 길이와 각 필드의 역할은 [TCP 헤더 구조](../전송과-소켓/01-TCP.md#tcp-헤더-구조)에서 설명한다.
...
- 데이터가 송신 측에서 하위 계층으로 내려갈 때, 각 계층에서 헤더(그리고 때로는 트레일러)를 추가하는 과정임.
...
- 수신 측에서 데이터가 상위 계층으로 올라갈 때, 각 계층에서 헤더를 제거하는 과정임.
...
- 계층: 5. Session:
  ...
  - 설명: 세션 정보 포함 데이터
- 계층: 4. Transport:
  - PDU 명칭: Segment (TCP) / Datagram (UDP)
  - 설명: 포트 정보 포함
- 계층: 3. Network:
  - PDU 명칭: Packet
  - 설명: IP 주소 포함
