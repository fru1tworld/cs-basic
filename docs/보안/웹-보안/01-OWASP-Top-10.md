...
OWASP(Open Web Application Security Project)는 웹 애플리케이션 보안을 위한 비영리 재단으로, 주요 보안 위험 10가지를 주기적으로 발표한다. 이 글은 OWASP Top 10:2021의 A01부터 A10까지를 코드 사례와 함께 살펴본다. 이미 공개된 2025판은 분류와 순서가 다르므로, 버전을 비교할 때는 [OWASP Top 10:2025](https://owasp.org/Top10/2025/)를 참고한다.
...
- 순위: 5:
  ...
  - 변경: XXE 통합
- 순위: 6:
  - 2017: Security Misconfiguration
  - 2021: Vulnerable Components
  - 변경: -
- 순위: 7:
  - 2017: XSS
  - 2021: Authentication Failures
  - 변경: -
- 순위: 8:
  - 2017: Insecure Deserialization
  - 2021: Software & Data Integrity
  - 변경: 확장
- 순위: 9:
  - 2017: Known Vulnerabilities
  - 2021: Logging & Monitoring Failures
  - 변경: -
- 순위: 10:
  - 2017: Insufficient Logging
  - 2021: SSRF
  - 변경: 신규
...
접근 제어는 사용자가 부여받은 권한 안에서만 행동하도록 제한한다. 이 검증이 빠지면 다른 사용자의 정보를 조회·수정·삭제하거나, 관리자처럼 권한 밖의 기능을 실행할 수 있다. 아래 예제는 요청한 사용자 ID와 현재 로그인한 사용자의 권한을 확인하지 않는 경우를 보여준다.

### 취약점 유형

- 수직적 권한 상승: 일반 사용자가 관리자 기능에 접근
- 수평적 권한 상승: 다른 사용자의 데이터에 접근
- IDOR (Insecure Direct Object Reference): 직접 객체 참조를 통한 접근
...
이전의 "Sensitive Data Exposure"에서 이름이 바뀐 분류로, 암호화 실패에 따른 민감 정보 노출을 다룬다. 데이터를 평문으로 전송하거나 약한 알고리즘과 부적절한 키 관리를 사용하는 경우가 여기에 해당한다.

### 취약점 유형

- 평문 데이터 전송: HTTPS 미사용
- 약한 암호화 알고리즘: MD5, SHA1, DES 사용
- 하드코딩된 암호화 키
- 부적절한 키 관리
...
신뢰할 수 없는 입력을 SQL, NoSQL, OS 명령어, LDAP 등의 명령이나 쿼리에 섞으면 인터프리터가 입력을 명령의 일부로 해석할 수 있다. Injection은 이 경계를 무너뜨리는 공격이며, 2021년 분류에는 XSS도 포함된다.
...
2021년에 추가된 이 분류는 설계와 아키텍처 자체의 보안 결함을 다룬다. 설계에 필요한 제어가 없다면 그 설계를 그대로 정확히 구현해도 문제가 남는다. 예측 가능한 비밀번호 재설정 토큰, 제한 없는 로그인 시도, 동시성 제어 없는 이체가 아래의 사례다.
...
- 보안 설정이 잘못되거나, 불필요한 기능이 활성화되어 있거나, 기본 계정/비밀번호가 변경되지 않은 경우 발생함. 2021년 버전에서는 XML External Entities (XXE)가 이 카테고리에 통합되었음.
...
- 취약점이 알려진 컴포넌트(라이브러리, 프레임워크)를 사용하거나 더 이상 지원되지 않는 버전을 사용할 때 발생함.
...
- 이전의 "Broken Authentication"으로, 사용자 인증 및 세션 관리 관련 취약점을 다룸.
...
- 2021년에 새로 추가된 카테고리로, 이전의 "Insecure Deserialization"을 확장하여 소프트웨어 업데이트, CI/CD 파이프라인, 역직렬화 관련 무결성 문제를 다룸.
...
이전의 "Insufficient Logging & Monitoring"에 해당하는 분류다. 보안 이벤트를 기록하지 않으면 공격 징후를 파악하기 어렵고, 기록하더라도 모니터링과 알림이 없으면 대응이 늦어진다. 다음 예제는 인증 성공·실패와 접근 거부, 의심스러운 활동을 기록하고 심각한 이벤트에는 알림을 보내는 흐름을 보여준다.
...
- 2021년에 새로 추가된 카테고리로, 서버가 사용자 제공 URL을 검증 없이 요청할 때 발생함.
