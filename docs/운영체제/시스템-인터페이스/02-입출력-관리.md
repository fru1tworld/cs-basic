...
I/O(Input/Output)는 컴퓨터와 디스크, 네트워크, 터미널 같은 외부 장치 사이의 데이터 전송이다. 애플리케이션은 시스템 콜로 커널에 요청하고, 커널은 VFS와 ext4, NFS, socket, device 등의 경로를 거쳐 장치 드라이버에 작업을 전달한다. 드라이버 아래에는 SSD, NIC, UART, GPU 같은 하드웨어가 있다.

### I/O의 특성

- 특성: 속도 격차:
  - 설명: CPU: ~ns, RAM: ~ns, SSD: ~us, HDD: ~ms, Network: ~ms
- 특성: 예측 불가능:
  - 설명: 네트워크 지연, 디스크 seek 시간 가변
- 특성: 비용:
  - 설명: I/O 대기 동안 CPU 유휴 상태
...
Blocking I/O에서는 호출한 함수가 완료될 때까지 스레드가 대기한다. 예를 들어 `read()`가 데이터를 기다리는 동안 호출자는 블록되고, 하드웨어에서 읽은 데이터를 커널이 반환하면 다음 명령을 실행한다.
...
Non-Blocking I/O는 데이터를 기다리며 호출자를 붙잡아 두지 않는다. `read()` 시점에 데이터가 준비되지 않았다면 `EAGAIN` 또는 `EWOULDBLOCK`을 반환하므로, 애플리케이션은 다른 작업을 하다가 다시 읽기를 시도할 수 있다. 아래 코드는 `O_NONBLOCK`을 설정하고 데이터가 없을 때 재시도하는 예다.
...
- 특성: CPU 사용:
  - Blocking: 대기 중 유휴
  - Non-Blocking: Polling 시 CPU 사용
- 특성: 프로그래밍 복잡도:
  - Blocking: 간단
  - Non-Blocking: 복잡
- 특성: 동시성:
  - Blocking: 멀티스레드 필요
  - Non-Blocking: 단일 스레드 가능
- 특성: 적합한 경우:
  - Blocking: 간단한 I/O
  - Non-Blocking: 다중 I/O 처리
...
연결마다 스레드를 하나씩 두면 Client 1, 2, 3을 각각 Thread 1, 2, 3이 처리한다. 연결이 늘어날수록 스레드 생성, 문맥 교환, 메모리 사용 비용도 커진다.

I/O Multiplexing은 하나의 스레드가 여러 연결의 I/O 상태를 함께 감시하는 방식이다. `select`, `poll`, `epoll`, `kqueue`로 준비된 연결을 확인하므로 연결마다 대기 스레드를 둘 필요가 없다.

### select()

- 가장 오래된 I/O Multiplexing 메커니즘임.
...
- Linux의 고성능 I/O Multiplexing 메커니즘임.
...
- BSD 계열 시스템의 고성능 I/O Multiplexing임.
...
- CPU 개입 없이 장치가 직접 메모리에 접근하여 데이터를 전송함.
...
CPU는 먼저 DMA 컨트롤러에 소스와 목적지 주소, 전송 크기, 읽기·쓰기 방향을 설정한다. DMA 컨트롤러가 버스 제어권을 얻어 장치와 메모리 사이에서 데이터를 전송하는 동안 CPU는 다른 작업을 수행할 수 있다. 전송이 끝나면 인터럽트로 CPU에 완료를 알린다.

### DMA 장점

- 장점: CPU 부하 감소:
  - 설명: 대용량 전송에서 CPU 사이클 절약
- 장점: 높은 처리량:
  - 설명: 버스 대역폭을 효율적으로 활용
- 장점: 병렬 처리:
  - 설명: CPU가 다른 작업 수행 가능
...
- [프로세스 간 통신](01-프로세스-간-통신.md)
- [POSIX 기초](03-POSIX-기초.md)
