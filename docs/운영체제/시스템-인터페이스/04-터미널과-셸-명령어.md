...
터미널에서 시스템을 다루려면 명령어를 해석하는 셸의 역할부터 이해해야 한다. 여기서는 셸별 특징과 기본 문법을 살펴보고, 프로세스와 자원 사용량을 확인하는 명령어로 이어간다.

## 셸 종류와 특징

### 셸의 역할

사용자가 입력한 명령어는 셸(Shell)이 해석하며, 실행된 프로그램은 커널(Kernel)을 통해 하드웨어를 사용한다. 셸은 이 과정에서 환경 변수를 관리하고, 파이프라인과 리다이렉션으로 명령어의 입출력을 연결한다.

여러 명령어를 반복해서 실행해야 할 때는 셸 스크립트로 묶을 수 있다. 대화형 작업에서는 작업 제어와 명령어 히스토리를 이용해 실행 중인 작업을 관리하거나 이전 명령어를 다시 불러온다.
...
- 셸: fish:
  - 개발: 2005
  - 특징: 사용자 친화적, 컬러 문법
  - 기본 설정 파일: ~/.config/fish/config.fish
- 셸: dash:
  - 개발: 2002
  - 특징: 경량, 빠른 스크립트 실행
  - 기본 설정 파일: /etc/profile

### Bash (Bourne Again Shell)

- 가장 널리 사용되는 셸로, Linux 배포판의 기본 셸임.
...
- macOS Catalina 이후 기본 셸. bash와 호환되며 더 강력한 기능을 제공함.
...
- 사용자 친화적인 설계로, 설정 없이도 강력한 기능을 제공함.
...
프로세스 상태는 `ps`와 `top`으로 확인하고, 프로세스가 사용하는 파일이나 네트워크 연결은 `lsof`, `netstat`, `ss`로 따라갈 수 있다. 이후 로그 내용을 좁힐 때는 `grep`이나 `ripgrep`으로 검색하고, 필요한 필드를 추출하거나 가공할 때는 `awk`와 `sed`를 사용한다.
...
앞에서 사용한 명령어를 스크립트로 묶으면 반복 작업을 자동화할 수 있다. 아래 예제는 Bash를 기준으로 변수, 조건문, 반복문, 함수를 차례로 살펴본 뒤 로그 분석과 백업, 헬스체크에 적용한다.
...
시스템 상태를 확인할 때는 CPU, 메모리, 디스크, 네트워크를 나누어 살펴본다. 전체 자원 사용량을 먼저 확인하고 프로세스별 사용량과 로그를 함께 보면, 어떤 작업이 자원을 사용하고 있는지 더 구체적으로 좁힐 수 있다.
...
- 문서 작성일: 2024년 12월
- 참고: 이 문서는 GNU Bash, POSIX 표준, Linux man-pages를 기반으로 작성되었음.
