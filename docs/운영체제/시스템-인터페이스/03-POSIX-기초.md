...
이 글은 POSIX.1-2017 표준을 기준으로 파일, 프로세스, 시그널을 다루는 인터페이스를 살펴본다.

## POSIX 표준 개요

### POSIX란?

POSIX(Portable Operating System Interface)는 IEEE에서 정의한 UNIX 계열 운영체제의 표준 인터페이스다. 시스템마다 다른 기능을 공통 API로 다룰 수 있게 해 시스템 프로그래밍 방식을 통일하고, 코드의 이식성과 시스템 간 호환성을 높이는 데 목적이 있다.

POSIX를 준수하거나 관련 인터페이스를 제공하는 시스템은 다음과 같다.
...
POSIX.1-2017(IEEE Std 1003.1-2017)은 기본 정의부터 C API와 셸 명령어까지 다룬다. 필요한 내용을 찾을 때는 다음 구분을 기준으로 보면 된다.

| 구성 | 다루는 내용 |
| --- | --- |
| Base Definitions (XBD) | 헤더 파일, 데이터 타입, 상수 정의 |
| System Interfaces (XSH) | `fork`, `exec`, `open`, `read`, `write` 등의 C 언어 API |
| Shell & Utilities (XCU) | `sh`, `ls`, `grep`, `awk` 등의 셸 명령어 |
| Rationale (XRAT) | 표준을 결정한 근거 |
...
파일 디스크립터는 I/O 대상을 가리키는 정수다. 프로세스는 자신의 파일 디스크립터 테이블을 통해 파일이나 소켓에 접근하며, 표준 입출력에는 다음 번호를 사용한다.
...
예를 들어 0, 1, 2번은 터미널(`tty`)에 연결하고, 3번은 네트워크 소켓, 4번은 `/var/log/app.log` 파일에 연결할 수 있다. 같은 정수 인터페이스를 사용하더라도 실제로 연결된 대상은 서로 다르다.
...
- 시그널은 프로세스에 비동기적으로 전달되는 소프트웨어 인터럽트임.
...
### Async-Signal-Safe 함수

- 시그널 핸들러에서 안전하게 호출할 수 있는 함수 목록임.
...
- 문서 작성일: 2024년 12월
- 참고: 이 문서는 POSIX.1-2017, Linux man-pages를 기반으로 작성되었음.

## 관련 학습

- [입출력 관리](02-입출력-관리.md)
- [터미널과 셸 명령어](04-터미널과-셸-명령어.md)
