...
SQL 명령어는 구조를 정의하는지, 데이터를 조작하는지, 권한이나 트랜잭션을 제어하는지에 따라 크게 4가지로 분류한다. 먼저 각 명령의 역할을 살펴보고, 뒤에서 조회에 쓰는 조인과 서브쿼리를 다룬다.
...
- DDL은 데이터베이스 스키마를 정의하거나 수정하는 명령어임.
...
트랜잭션 전체를 취소하지 않고 일부 작업만 되돌리려면 SAVEPOINT를 둔다. 아래 예에서는 주문과 주문 항목을 만든 뒤 `items_added`를 지정한다. 결제 처리에서 오류가 나면 이 지점까지만 롤백하고, 앞서 만든 주문을 유지한 채 다른 결제 처리를 이어간다.
...
사용자마다 최근 주문을 3개씩 가져오려면 서브쿼리가 현재 사용자를 참조해야 한다. LATERAL을 사용한 아래 쿼리는 바깥쪽 `u.id`로 주문을 걸러 낸 뒤, 사용자별로 정렬과 LIMIT을 적용한다.
...
- [SQL의 의미와 평가 순서](01-SQL의-의미와-평가-순서.md)
