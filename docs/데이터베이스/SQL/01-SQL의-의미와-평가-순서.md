...
SQL은 원하는 결과를 선언하면 DBMS가 실행 방법을 결정하는 관계형 데이터베이스의 표준 쿼리 언어다. 실행 방법을 맡기더라도 각 구문이 뜻하는 바는 정확히 알아야 한다. 특히 논리적 처리 순서와 NULL의 의미를 이해해야 필터링이나 집계 결과가 예상과 달라지는 이유를 찾을 수 있다.

## SQL의 논리적 처리 순서

SQL은 SELECT로 작성하기 시작하지만 논리적으로는 FROM에서 대상 테이블과 조인을 정한 뒤 행을 필터링한다. 아래 순서로 이해하면 WHERE가 개별 행을, HAVING이 그룹을 거르는 이유도 연결된다.
...
## NULL 처리와 3-Valued Logic

SQL은 true, false 외에 unknown을 포함하는 3값 논리를 사용한다. NULL을 비교하면 unknown이 될 수 있으므로, AND와 OR로 조건을 결합할 때도 이 값을 고려해야 한다.
...
NOT은 TRUE를 FALSE로, FALSE를 TRUE로 바꾸지만 UNKNOWN은 그대로 유지한다. 아래 예처럼 NULL 비교에 NOT을 붙이는 것만으로는 일반적인 값 비교가 되지 않는다.
...
- [SQL 명령과 쿼리 작성](02-SQL-명령과-쿼리-작성.md)
