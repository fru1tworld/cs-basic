...
옵티마이저는 실행 계획을 모두 실행해 본 뒤 고를 수 없으므로 각 계획의 비용을 미리 추정한다. 이 추정이 실제 작업량과 맞아야 적절한 계획을 선택할 수 있다. 특히 처리할 행 수를 잘못 예상하면 스캔이나 조인 방법도 잘못 선택할 수 있어, 비용 추정과 카디널리티 추정을 함께 살펴봐야 한다.
...
자주 등장하는 A, B, C 같은 값은 MCV에 빈도를 따로 기록하고, 나머지 값들의 분포는 히스토그램으로 나타낸다. 조회할 값이 MCV에 있으면 기록된 빈도를 직접 사용하며, 없으면 히스토그램에서 분포를 추정한다. 이렇게 빈번한 값을 분리하면 편향된 분포를 하나의 균등한 분포로 취급하지 않을 수 있다.
