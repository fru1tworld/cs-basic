...
ORM(Object-Relational Mapping)은 프로그램의 객체와 관계형 데이터베이스의 테이블 사이를 매핑한다. 예를 들어 `id: 1`, `name: Kim`, `email: ...`을 가진 User 객체를 `users` 테이블의 `id = 1`, `name = 'Kim'`, `email = ...`인 행과 연결한다.

이 매핑을 자동화하면 SQL을 직접 작성하는 일을 줄이고 DB 벤더에 대한 의존을 낮출 수 있다. 생산성과 타입 안정성에도 도움이 되지만, ORM 자체를 익혀야 하고 복잡한 쿼리를 최적화하기 어려울 수 있다. 성능 오버헤드나 N+1 문제를 이해하려면 객체 접근이 어떤 SQL로 이어지는지도 살펴봐야 한다.

### 주요 ORM 프레임워크

- 언어: Java:
  - ORM 프레임워크: Hibernate, JPA, MyBatis
- 언어: Python:
  - ORM 프레임워크: SQLAlchemy, Django ORM
- 언어: JavaScript/TypeScript:
  - ORM 프레임워크: TypeORM, Prisma, Sequelize
- 언어: Ruby:
  - ORM 프레임워크: ActiveRecord
- 언어: Go:
  - ORM 프레임워크: GORM, Ent
- 언어: .NET:
  - ORM 프레임워크: Entity Framework

## ORM 패턴

### Active Record 패턴

- 객체가 데이터베이스 행을 감싸고, CRUD 메서드를 직접 포함함.
...
- 도메인 객체와 데이터베이스를 분리하고, 별도의 매퍼 클래스가 변환을 담당함.
...
위 예에서 User 엔티티는 `id`, `name`, `email`을 가진 도메인 모델이고, UserRepository가 `findById()`, `save()`, `delete()` 같은 데이터 접근을 맡는다. 매퍼가 객체와 `users` 테이블 사이에서 SQL 생성과 변환을 담당하므로 비즈니스 로직과 저장 로직을 분리할 수 있다. 테스트와 변경이 유연해지는 대신 구성 요소와 코드가 늘어난다.

### Active Record vs Data Mapper

- 특성: 결합도:
  - Active Record: 모델 = 테이블
  - Data Mapper: 모델 ≠ 테이블
- 특성: 복잡성:
  - Active Record: 낮음
  - Data Mapper: 높음
- 특성: 테스트:
  - Active Record: 어려움 (DB 의존)
  - Data Mapper: 용이 (Mock 가능)
- 특성: 적합한 경우:
  - Active Record: 간단한 CRUD
  - Data Mapper: 복잡한 도메인
- 특성: 대표 프레임워크:
  - Active Record: Rails, Django
  - Data Mapper: JPA, SQLAlchemy
...
이 예에서는 게시글 100개를 읽는 쿼리 1번에 작성자를 조회하는 쿼리 100번이 더해져 총 101번 실행된다. 첫 번째 게시글의 `id = 1` 작성자를 읽은 뒤에도 세 번째 게시글에서 같은 작성자를 다시 조회할 수 있다. 객체 속성을 읽는 반복문이 데이터베이스 왕복을 반복하는 구조다.
...
- 예시: 사용자와 주문
- User, ○←, Order
- "사용자는 0개 이상의 주문을 가질 수 있음"
- "주문은 정확히 1명의 사용자에게 속함"
