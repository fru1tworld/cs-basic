...
관계형 모델은 1970년 Edgar F. Codd가 제안한 모델로, 데이터를 릴레이션(테이블)으로 표현하고 관계 대수로 조작한다. 집합 이론과 1차 술어 논리를 기반으로 데이터를 다루면서, 논리적으로 보이는 형태와 물리적으로 저장하는 방법을 분리했다. 이 분리가 데이터 독립성의 바탕이다.

## 릴레이션의 수학적 정의

- 릴레이션은 집합론적으로 다음과 같이 정의됨:
...
- Codd는 1985년 "진정한" 관계형 DBMS가 갖춰야 할 12가지 규칙을 제시했음:

- 규칙: 0:
  - 이름: 기본 규칙
  - 설명: 관계형 DBMS는 관계적 기능만으로 데이터를 관리해야 함
- 규칙: 1:
  - 이름: 정보 규칙
  - 설명: 모든 정보는 테이블 형태로 표현
- 규칙: 2:
  - 이름: 보장된 접근 규칙
  - 설명: 테이블명 + 기본키 + 컬럼명으로 모든 데이터 접근 가능
- 규칙: 3:
  - 이름: NULL 체계적 처리
  - 설명: NULL은 "알 수 없음"을 일관되게 표현
- 규칙: 4:
  - 이름: 관계형 온라인 카탈로그
  - 설명: 메타데이터도 테이블로 저장
- 규칙: 5:
  - 이름: 포괄적 데이터 하위 언어
  - 설명: 모든 데이터 조작을 지원하는 언어 필요
- 규칙: 6:
  - 이름: 뷰 갱신 규칙
  - 설명: 이론적으로 갱신 가능한 뷰는 갱신 가능해야 함
- 규칙: 7:
  - 이름: 고수준 삽입/갱신/삭제
  - 설명: 집합 단위 연산 지원
- 규칙: 8:
  - 이름: 물리적 데이터 독립성
  - 설명: 물리적 구조 변경이 논리적 구조에 영향 없음
- 규칙: 9:
  - 이름: 논리적 데이터 독립성
  - 설명: 논리적 구조 변경이 응용에 영향 없음
- 규칙: 10:
  - 이름: 무결성 독립성
  - 설명: 무결성 제약이 응용 프로그램과 독립
- 규칙: 11:
  - 이름: 분산 독립성
  - 설명: 분산 환경에서도 동일하게 동작
- 규칙: 12:
  - 이름: 비전복 규칙
  - 설명: 하위 수준 접근이 무결성을 우회할 수 없음

- 실무적 의미: 현대 RDBMS도 12규칙을 완전히 만족하지 못함. 특히 규칙 6(뷰 갱신)과 규칙 9(논리적 독립성)는 여전히 제한적임.

## 관계 대수 (Relational Algebra)

- 관계 대수는 릴레이션을 입력받아 새로운 릴레이션을 출력하는 절차적 쿼리 언어임.
...
- 관계 미적분은 선언적 쿼리 언어로, "무엇을" 원하는지만 명시함.
...
관계 대수는 "어떻게" 계산할지 연산으로 나타내고, 관계 미적분은 "무엇을" 원하는지 조건으로 나타낸다. 표현 방법은 다르지만 Codd의 정리에 따르면 관계 대수와 안전한(safe) 관계 미적분의 표현력은 같다.

SQL은 원하는 결과를 선언한다는 점에서 관계 미적분에 가깝다. DBMS는 SQL을 구문 분석한 뒤 관계 대수 표현으로 바꾸고, 옵티마이저를 거쳐 실행 계획을 만든다. 선언한 결과를 실제 연산으로 옮기는 과정이다.
