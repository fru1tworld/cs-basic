...
복제(Replication)는 같은 데이터를 여러 노드에 두는 방식이다. 한 노드에 장애가 나더라도 다른 노드로 전환하는 페일오버를 통해 서비스를 이어갈 수 있고, Read Replica에 읽기 부하를 나누어 처리량을 늘릴 수도 있다.

복제본을 어디에 두는지도 목적에 따라 달라진다. 사용자 가까이에 배치하면 네트워크 지연을 줄이고, 다른 지역에 두면 데이터센터 장애에 대비한 재해 복구에 활용할 수 있다.
...
단일 DB의 저장 용량, 쓰기 성능, 연결 수가 한계에 도달하면 데이터를 여러 DB로 나눌 수 있다. 샤딩(Sharding)은 이처럼 각 샤드가 데이터의 일부를 맡는 방식이다. Router가 A-F는 Shard1, G-L은 Shard2, W-Z는 ShardN으로 보내는 식으로 대상 샤드를 고른다. 각 샤드에는 별도로 복제본을 둘 수 있다.
...
노드 수가 바뀔 때 재배치를 줄이려면 Consistent Hashing(일관된 해싱)을 사용할 수 있다. Node A, B, C를 해시 링에 배치해 추가되거나 제거된 노드와 관련된 키만 옮기며, 가상 노드(Virtual Node)로 분포의 불균형을 줄인다. 아래 구현은 각 실제 노드에 여러 가상 노드를 두고 정렬된 해시 위치에서 담당 노드를 찾는다.
...
- 네트워크 분할이 발생한 상황에서는 선형화 가능한 일관성과 모든 정상 노드에서의 요청 완료를 동시에 보장할 수 없음.
- C는 읽기와 쓰기가 실시간 순서를 따르는 하나의 복사본처럼 보이는 성질이며, 모든 노드의 물리적 값이 매 순간 같다는 뜻은 아님.
- 분할 상황에서 일관성을 우선하면 일부 요청을 거절하거나 기다리게 할 수 있음. 응답을 계속 제공하면 서로 다른 값을 관찰할 수 있음.
- PostgreSQL, MySQL, MongoDB, Cassandra, DynamoDB, HBase, ZooKeeper, CouchDB, DNS를 제품 이름만으로 CP, AP, CA에 고정 분류하지 않아야 함. 복제 구성, 읽기와 쓰기 옵션, 실패 처리 정책에 따라 실제 보장이 달라짐.
- 대표 설명과 적용 조건은 [CAP와 일관성 모델](../../분산시스템/합의와-일관성/01-CAP와-일관성-모델.md)에 통합함.
