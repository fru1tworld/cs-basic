...
친구 관계를 여러 단계 따라가려면 관계형 데이터베이스에서는 같은 테이블을 반복해서 조인한다. 아래 SQL과 Cypher 예제는 이런 관계 탐색을 각각 어떻게 표현하는지 보여준다.
...
- 특성: 데이터 모델:
  ...
  - Graph DB: 노드, 관계, 속성
- 특성: 관계 표현:
  - RDBMS: Foreign Key, JOIN
  - Graph DB: 직접 연결
- 특성: 조인 성능:
  - RDBMS: O(n log n) ~ O(n²)
  - Graph DB: O(k) - 관계 수에 비례
- 특성: 스키마:
  - RDBMS: 고정 스키마
  - Graph DB: 유연한 스키마
- 특성: 적합한 쿼리:
  - RDBMS: 집계, 범위 검색
  - Graph DB: 경로 탐색, 패턴 매칭
...
Property Graph Model에서는 정점인 Node와 간선인 Relationship 모두에 속성을 둘 수 있다. Person 레이블을 가진 노드에 `name: "Kim"`, `age: 30`을 저장하고, Company 레이블을 가진 노드에는 `name: "ABC"`, `industry: "Tech"`를 저장하는 식이다.

두 노드는 방향이 있는 `WORKS_AT` 관계로 연결하고 관계 자체에 `since: 2020`을 담을 수 있다. `FRIEND`, `PURCHASED`처럼 관계의 종류를 타입으로 구분하며 `since`, `weight` 같은 속성은 그 관계를 설명한다.
