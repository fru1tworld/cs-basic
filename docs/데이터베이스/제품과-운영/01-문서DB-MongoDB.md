...
MongoDB는 BSON(Binary JSON) 형식으로 문서를 저장하는 문서 지향(Document-Oriented) NoSQL 데이터베이스다. 관계형 DB의 테이블과 행에 익숙하다면, 다음 대응 관계를 출발점으로 이해할 수 있다.
...
MongoDB 서버의 저장 엔진인 WiredTiger는 메모리의 Cache, WAL 로그인 Journal, 스냅샷인 Checkpoint를 사용해 데이터를 관리한다. 컬렉션과 인덱스 데이터는 `.wt` 파일에 저장한다.

### 복제 세트 (Replica Set)

Replica Set에서 Primary는 읽기와 쓰기를 받고 Secondary는 읽기를 처리한다. 변경 사항은 capped collection인 Oplog에 기록되어 복제되며, Primary에 장애가 발생하면 Secondary를 Primary로 승격하는 자동 페일오버가 이루어진다. 아래 예에서는 복제 세트를 구성하고 읽기 대상에 대한 선호도를 지정한다.
