...
mysql 클라이언트나 JDBC, PHP, Python에서 접속하면 Connection Layer가 연결과 스레드 풀, 인증 및 보안을 처리한다. SQL은 MySQL Server의 SQL Layer에서 Parser → Preprocessor → Optimizer → Executor 순서로 처리된다. 이전에 있던 Query Cache는 8.0에서 제거되었다.

실제 저장은 Storage Engine Layer가 맡는다. 기본 엔진인 InnoDB 외에 MyISAM, Memory, Archive 등이 있으며, 파일 시스템에는 데이터 파일과 Redo Log(`ib_log`), Undo Log, Binary Log를 둔다.
...
비동기 복제(Asynchronous Replication)는 Master의 Binary Log를 Slave에 전달하되, Slave의 수신을 기다리지 않고 쓰기에 응답한다. 반동기 복제(Semi-Synchronous Replication)는 최소 1개 Slave가 로그를 받았다는 ACK를 기다린 뒤 응답한다.

Group Replication(MGR)은 Node 1, 2, 3을 그룹으로 구성한다. 하나의 Primary와 여러 Secondary를 두는 Single-Primary 방식 또는 Multi-Master 방식으로 운영할 수 있다.
