...
데이터베이스가 필요한 데이터를 찾더라도 저장 매체에서 읽어 오는 데 시간이 걸린다. CPU와 메모리 사이에는 메모리 벽, 메모리와 디스크 사이에는 I/O 벽이라 부르는 속도 차이가 있기 때문이다. 특히 디스크 I/O는 주요 병목이 되므로, DBMS는 저장 계층의 특성에 맞춰 데이터를 배치하고 자주 읽는 페이지를 메모리에 보관한다.
...
## 버퍼 풀 관리 (Buffer Pool Management)

버퍼 풀은 디스크 페이지를 메모리의 프레임(Frame)에 올려 두고 재사용한다. 예를 들어 Page 1, 5, 3, 8, 2를 보관하고 있다면, 수정된 Page 5와 8은 dirty 상태이고 나머지는 clean 상태다.

어느 프레임에 페이지가 있는지는 페이지 테이블의 `page_id → frame_id` 매핑으로 찾는다. 다음 예에서 Page 1은 0번 프레임에 있고, Page 5는 1번 프레임에 있다.
...
`dirty`는 메모리와 디스크의 내용이 다른지를 나타낸다. `pin_count`는 현재 페이지를 사용하는 쿼리 수이며, 0이어야 교체할 수 있다. 따라서 Page 5는 수정 여부뿐 아니라 현재 사용 중이라는 점도 교체 시 고려해야 한다.
...
순차 프리페칭은 Page 1을 요청할 때 Page 2, 3, 4도 함께 읽는 방식이다. 다음 페이지를 계속 읽는 테이블 스캔에 효과적이며, OS와 DB 양쪽에서 수행할 수 있다. PostgreSQL에서는 `effective_io_concurrency` 설정을 함께 살펴볼 수 있다.

인덱스 스캔에서도 다음에 필요한 페이지를 예측해 미리 읽을 수 있다. B+Tree의 리프 노드를 순차적으로 접근하거나 Bitmap Index Scan을 수행하는 경우가 이에 해당한다.
