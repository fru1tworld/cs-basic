...
### SPRING-002
특정 기능을 구현한 클래스가 하나뿐이라면 직접 생성해서 사용해도 되지 않을까요? 이 경우에도 Spring Bean으로 관리하면 어떤 이점이 있나요?
...
### SPRING-008
Interceptor가 있어도 Servlet Filter가 필요한 이유는 무엇인가요? 각각 어떤 상황에 사용하는지 설명해 주세요.
...
### SPRING-019
Spring은 Annotation에 담긴 정보를 어떻게 읽고 처리하나요? Annotation이 실제 동작으로 이어지는 과정을 설명해 주세요.
