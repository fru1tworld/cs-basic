...
메시지와 이벤트의 전달 보장 수준인 'At-least-once', 'At-most-once', 'Exactly-once'는 어떤 차이가 있나요?
...
이벤트가 쌓이면 '현재 상태'를 재구성하는 데 시간이 오래 걸릴 수 있습니다. 이벤트 소싱에서 이 문제를 어떻게 해결할 수 있나요? (힌트: Snapshot)
...
CQRS와 이벤트 소싱을 함께 사용하면 각각의 역할이 어떻게 연결되며, 어떤 이점이 있나요?
...
단일 리더(Single-Leader) 복제 아키텍처 (Primary-Replica)에 대해 설명해 주세요.
...
Rate Limiting(속도 제한) 알고리즘(Token Bucket, Leaky Bucket, Sliding Window)에 대해 설명해주세요.

### SD-097

Circuit Breaker 패턴이란 무엇이며 어떻게 구현하나요?

### SD-098

Bulkhead 패턴과 격리 전략에 대해 설명해주세요.

### SD-099

Cache-Aside, Read-Through, Write-Through, Write-Behind 캐싱 전략의 차이점은 무엇인가요?

### SD-100

분산 ID 생성 전략(Snowflake, ULID, UUID v7)에 대해 설명해주세요.

### SD-101

Backpressure(역압) 처리 전략에 대해 설명해주세요.

### SD-102

URL Shortener 시스템을 설계한다면 어떤 구성요소와 고려사항이 필요한가요?
