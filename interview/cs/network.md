...
### NET-011
그렇다면 HTTP/3에서는 왜 UDP를 사용하나요? UDP가 보장하지 않는 신뢰성은 어떻게 확보하나요?
...
### NET-063
같은 네트워크에 있는 IPv4 장비와 IPv6 장비가 서로 통신할 수 있나요? 가능하다면 어떤 방법을 사용하나요?

### NET-064
IP는 송신자가 보낸 데이터가 수신자에게 정확히 전달되는 것을 보장하나요?
...
### NET-085
각 계층에서 다루는 데이터 단위를 무엇이라고 부르나요? 예를 들어 TCP에서는 Segment라는 이름을 사용합니다.
...
### NET-105
gRPC란 무엇이고, REST API와 비교했을 때 어떤 차이점이 있나요?

### NET-106
Protocol Buffers의 직렬화 방식과 JSON 대비 장단점을 설명해주세요.

### NET-107
gRPC의 스트리밍 방식(Unary, Server Streaming, Client Streaming, Bidirectional)을 설명해주세요.

### NET-108
CDN(Content Delivery Network)의 동작 원리와 사용 목적을 설명해주세요.

### NET-109
REST API 설계 원칙(RESTful 제약 조건)에 대해 설명해주세요.

### NET-110
API 버전 관리 전략(URL, Header, Query Parameter)의 장단점을 비교해주세요.

### NET-111
Rate Limiting의 구현 방식(Token Bucket, Leaky Bucket, Sliding Window)을 설명해주세요.
