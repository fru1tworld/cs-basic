...
CPU의 구조와 주요 구성 요소(ALU, 제어 유닛, 레지스터)를 설명해 주세요.

### ARCH-002

명령어 사이클(Instruction Cycle)의 단계(Fetch, Decode, Execute)를 설명해 주세요.
...
파이프라인 해저드(Pipeline Hazard)의 종류와 해결 방법을 설명해 주세요.
...
메모리 계층 구조(Memory Hierarchy)를 설명해 주세요.

### ARCH-010

레지스터, 캐시, 메인 메모리, 보조 기억장치의 속도와 용량을 비교해 주세요.
...
캐시 히트(Cache Hit)와 캐시 미스(Cache Miss)를 설명해 주세요.

### ARCH-016

캐시 매핑 방식(Direct Mapped, Fully Associative, Set Associative)의 차이점을 설명해 주세요.

### ARCH-017

캐시 교체 정책(LRU, LFU, FIFO 등)을 설명해 주세요.
...
L1, L2, L3 캐시의 차이점과 각각의 역할을 설명해 주세요.

### ARCH-020

캐시 일관성(Cache Coherence) 문제와 해결 방법을 설명해 주세요.
...
페이지 테이블(Page Table)의 역할과 구조를 설명해 주세요.
...
페이지 폴트(Page Fault)가 발생했을 때 처리 과정을 설명해 주세요.
...
메모리 버스(Memory Bus)와 대역폭(Bandwidth)을 설명해 주세요.
...
멀티코어 환경에서 캐시 일관성을 유지하는 프로토콜(MESI, MOESI 등)을 설명해 주세요.
...
NUMA(Non-Uniform Memory Access) 아키텍처를 설명해 주세요.

## 입출력 시스템

### ARCH-036

I/O 처리 방식(Programmed I/O, Interrupt-driven I/O, DMA)의 차이점을 설명해 주세요.

### ARCH-037

인터럽트(Interrupt)의 동작 원리와 종류를 설명해 주세요.

### ARCH-038

폴링(Polling)과 인터럽트 방식의 장단점을 비교해 주세요.
...
CPU 성능을 측정하는 지표(Clock Speed, IPC, CPI 등)를 설명해 주세요.
...
Amdahl의 법칙을 설명하고, 병렬화의 한계를 설명해 주세요.

### ARCH-044

벤치마크(Benchmark)의 종류와 성능 측정 방법을 설명해 주세요.
...
데이터베이스 쿼리 성능에 CPU 캐시가 미치는 영향을 설명해 주세요.
...
메모리 정렬(Memory Alignment)이 성능에 미치는 영향을 설명해 주세요.
...
최근 서버용 CPU의 발전 트렌드(코어 수 증가, 특수 명령어 등)를 설명해 주세요.
...
컴퓨터 구조 관점에서 클라우드 인프라의 특징과 고려 사항은 무엇인가요?
